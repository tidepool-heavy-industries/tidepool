use crate::error::BridgeError;
use crate::traits::{
    sealed::{FromHaskellSealed, ToHaskellSealed},
    FromHaskell, HaskellVisitor, ToHaskell,
};
use crate::{shapes, HaskellValue};
use tidepool_repr::{DataConId, DataConTable, Literal};

/// Resolve a fixed constructor by its compiler-published identity and verify
/// the representation arity expected by the bridge site.
pub fn get_qualified(table: &DataConTable, qualified_name: &str, arity: u32) -> Option<DataConId> {
    table
        .get_by_qualified_name(qualified_name)
        .filter(|id| table.get(*id).is_some_and(|con| con.rep_arity == arity))
}

const UNIT: &str = "GHC.Tuple.()";
const PAIR: &str = "GHC.Tuple.(,)";
const TRIPLE: &str = "GHC.Tuple.(,,)";
const INT: &str = "GHC.Types.I#";
const WORD: &str = "GHC.Types.W#";
const DOUBLE: &str = "GHC.Types.D#";
const CHAR: &str = "GHC.Types.C#";
const TRUE: &str = "GHC.Types.True";
const FALSE: &str = "GHC.Types.False";
const TEXT: &str = "Data.Text.Text";
const NOTHING: &str = "GHC.Maybe.Nothing";
const JUST: &str = "GHC.Maybe.Just";
const NIL: &str = "GHC.Types.[]";
const CONS: &str = "GHC.Types.:";
const LEFT: &str = "Data.Either.Left";
const RIGHT: &str = "Data.Either.Right";

fn unbox_exact<T: Copy>(
    value: &HaskellValue,
    constructor: Option<DataConId>,
    literal: impl Fn(&Literal) -> Option<T>,
) -> Option<T> {
    let mut current = value;
    loop {
        match current {
            HaskellValue::Lit(value) => return literal(value),
            HaskellValue::Con(id, fields) if constructor == Some(*id) && fields.len() == 1 => {
                current = &fields[0];
            }
            _ => return None,
        }
    }
}

/// Build a [`BridgeError::TypeMismatch`] naming `expected` and rendering
/// `got`'s actual shape. The one value-kind formatter for every `FromHaskell`
/// impl — hand-written here and derive-macro-generated
/// (`tidepool_bridge_derive::codegen`) alike — so the "what did we actually
/// get" wording can't drift between the two.
#[must_use]
pub fn type_mismatch(expected: &str, got: &HaskellValue) -> BridgeError {
    BridgeError::TypeMismatch {
        expected: expected.to_string(),
        got: value_shape(got),
    }
}

fn value_shape(value: &HaskellValue) -> String {
    match value {
        HaskellValue::Lit(l) => format!("Lit({:?})", l),
        HaskellValue::Con(id, _) => format!("Con({:?})", id),
        HaskellValue::ByteArray(bs) => match bs.lock() {
            Ok(b) => format!("ByteArray(len={})", b.len()),
            Err(_) => "ByteArray(poisoned)".to_string(),
        },
    }
}

/// Preserve the matched outer field and the actual nested value identity when
/// a derived decoder fails below that field. The nested [`BridgeError`] still
/// carries the machine-readable cause; this context makes constructor-table
/// mismatches diagnosable without reproducing the live heap.
#[must_use]
pub fn field_decode_error(
    constructor: &str,
    field: usize,
    value: &HaskellValue,
    table: &DataConTable,
    source: BridgeError,
) -> BridgeError {
    let observed = match value {
        HaskellValue::Con(id, _) => table
            .get(*id)
            .map(|data_con| {
                data_con
                    .qualified_name
                    .clone()
                    .unwrap_or_else(|| data_con.name.clone())
            })
            .unwrap_or_else(|| format!("Con({id:?})")),
        _ => value_shape(value),
    };
    BridgeError::FieldDecode {
        constructor: constructor.to_string(),
        field,
        observed,
        source: Box::new(source),
    }
}

impl<T> FromHaskellSealed for std::marker::PhantomData<T> {}
impl<T> ToHaskellSealed for std::marker::PhantomData<T> {}

impl<T> FromHaskell for std::marker::PhantomData<T> {
    fn from_value(value: &HaskellValue, table: &DataConTable) -> Result<Self, BridgeError> {
        match value {
            HaskellValue::Con(id, fields) if get_qualified(table, UNIT, 0) == Some(*id) => {
                if fields.is_empty() {
                    Ok(std::marker::PhantomData)
                } else {
                    Err(BridgeError::ArityMismatch {
                        con: *id,
                        expected: 0,
                        got: fields.len(),
                    })
                }
            }
            HaskellValue::Con(id, _) => Err(BridgeError::UnknownDataCon(*id)),
            _ => Err(type_mismatch("Con", value)),
        }
    }
}

impl<T> ToHaskell for std::marker::PhantomData<T> {
    fn visit(
        &self,
        table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        // PhantomData has no Haskell representation; derived fields still need
        // an arity-0 placeholder. Require "()" because another nullary
        // constructor would corrupt downstream decoding.
        let id = get_qualified(table, UNIT, 0)
            .ok_or_else(|| BridgeError::UnknownDataConName("()".into()))?;
        visitor.begin_constructor(id, 0)?;
        visitor.end_constructor()
    }
}

// Box

// HaskellValue identity — pass through without conversion.
impl FromHaskellSealed for HaskellValue {}
impl ToHaskellSealed for HaskellValue {}

impl ToHaskell for HaskellValue {
    fn visit(
        &self,
        _table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        enum Event<'a> {
            Value(&'a HaskellValue),
            End,
        }
        let mut events = vec![Event::Value(self)];
        while let Some(event) = events.pop() {
            match event {
                Event::End => visitor.end_constructor()?,
                Event::Value(HaskellValue::Lit(literal)) => visitor.literal(literal.clone())?,
                Event::Value(HaskellValue::ByteArray(bytes)) => visitor.byte_array(
                    bytes
                        .lock()
                        .unwrap_or_else(std::sync::PoisonError::into_inner)
                        .clone(),
                )?,
                Event::Value(HaskellValue::Con(id, fields)) => {
                    visitor.begin_constructor(*id, fields.len())?;
                    events.push(Event::End);
                    events.extend(fields.iter().rev().map(Event::Value));
                }
            }
        }
        Ok(())
    }
}

impl FromHaskell for HaskellValue {
    fn from_value(value: &HaskellValue, _table: &DataConTable) -> Result<Self, BridgeError> {
        Ok(value.clone())
    }
}

impl<T> FromHaskellSealed for Box<T> {}
impl<T> ToHaskellSealed for Box<T> {}

impl<T: FromHaskell> FromHaskell for Box<T> {
    fn from_value(value: &HaskellValue, table: &DataConTable) -> Result<Self, BridgeError> {
        T::from_value(value, table).map(Box::new)
    }
}

impl<T: ToHaskell> ToHaskell for Box<T> {
    fn visit(
        &self,
        table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        (**self).visit(table, visitor)
    }
}

// Unit

impl FromHaskellSealed for () {}
impl ToHaskellSealed for () {}

impl ToHaskell for () {
    fn visit(
        &self,
        table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        let id = get_qualified(table, UNIT, 0)
            .ok_or_else(|| BridgeError::UnknownDataConName("()".into()))?;
        visitor.begin_constructor(id, 0)?;
        visitor.end_constructor()
    }
}

impl FromHaskell for () {
    fn from_value(value: &HaskellValue, table: &DataConTable) -> Result<Self, BridgeError> {
        match value {
            // Mirror `to_value`: only the `()` constructor decodes to unit.
            // Accepting ANY nullary con (`Nothing`, `False`, `[]`, a user
            // nullary constructor, ...) silently corrupts downstream decode —
            // exactly the failure mode `to_value`'s own comment warns about
            // for the encode direction (#F6).
            HaskellValue::Con(id, fields)
                if fields.is_empty() && get_qualified(table, UNIT, 0) == Some(*id) =>
            {
                Ok(())
            }
            _ => Err(type_mismatch("()", value)),
        }
    }
}

// Primitives

/// Bridges Rust `i64` to Haskell `Int#` literal.
/// Also transparently unwraps `I#(n)` (boxed Int).
impl FromHaskellSealed for i64 {}
impl ToHaskellSealed for i64 {}

impl FromHaskell for i64 {
    fn from_value(value: &HaskellValue, table: &DataConTable) -> Result<Self, BridgeError> {
        unbox_exact(
            value,
            get_qualified(table, INT, 1),
            |literal| match literal {
                Literal::LitInt(value) => Some(*value),
                _ => None,
            },
        )
        .ok_or_else(|| type_mismatch("LitInt or I#", value))
    }
}

impl ToHaskell for i64 {
    fn visit(
        &self,
        table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        let id = get_qualified(table, INT, 1)
            .ok_or_else(|| BridgeError::UnknownDataConName("I#".into()))?;
        visitor.begin_constructor(id, 1)?;
        visitor.literal(Literal::LitInt(*self))?;
        visitor.end_constructor()
    }
}

/// Bridges Rust `u64` to Haskell `Word#` literal.
/// Also transparently unwraps `W#(n)` (boxed Word).
impl FromHaskellSealed for u64 {}
impl ToHaskellSealed for u64 {}

impl FromHaskell for u64 {
    fn from_value(value: &HaskellValue, table: &DataConTable) -> Result<Self, BridgeError> {
        unbox_exact(
            value,
            get_qualified(table, WORD, 1),
            |literal| match literal {
                Literal::LitWord(value) => Some(*value),
                _ => None,
            },
        )
        .ok_or_else(|| type_mismatch("LitWord or W#", value))
    }
}

impl ToHaskell for u64 {
    fn visit(
        &self,
        table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        let id = get_qualified(table, WORD, 1)
            .ok_or_else(|| BridgeError::UnknownDataConName("W#".into()))?;
        visitor.begin_constructor(id, 1)?;
        visitor.literal(Literal::LitWord(*self))?;
        visitor.end_constructor()
    }
}

/// Bridges Rust `f64` to Haskell `Double#` literal.
/// Also transparently unwraps `D#(n)` (boxed Double).
impl FromHaskellSealed for f64 {}
impl ToHaskellSealed for f64 {}

impl FromHaskell for f64 {
    fn from_value(value: &HaskellValue, table: &DataConTable) -> Result<Self, BridgeError> {
        unbox_exact(
            value,
            get_qualified(table, DOUBLE, 1),
            |literal| match literal {
                Literal::LitDouble(bits) => Some(f64::from_bits(*bits)),
                _ => None,
            },
        )
        .ok_or_else(|| type_mismatch("LitDouble or D#", value))
    }
}

impl ToHaskell for f64 {
    fn visit(
        &self,
        table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        let id = get_qualified(table, DOUBLE, 1)
            .ok_or_else(|| BridgeError::UnknownDataConName("D#".into()))?;
        visitor.begin_constructor(id, 1)?;
        visitor.literal(Literal::LitDouble(self.to_bits()))?;
        visitor.end_constructor()
    }
}

/// Bridges Rust `i32` to Haskell `Int#` literal.
/// Returns error on overflow/underflow.
impl FromHaskellSealed for i32 {}
impl ToHaskellSealed for i32 {}

impl FromHaskell for i32 {
    fn from_value(value: &HaskellValue, table: &DataConTable) -> Result<Self, BridgeError> {
        let n = i64::from_value(value, table)?;
        if n < i32::MIN as i64 || n > i32::MAX as i64 {
            return Err(BridgeError::TypeMismatch {
                expected: "i32 in range [-2147483648, 2147483647]".to_string(),
                got: format!("LitInt({})", n),
            });
        }
        Ok(n as i32)
    }
}

impl ToHaskell for i32 {
    fn visit(
        &self,
        table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        (*self as i64).visit(table, visitor)
    }
}

/// Bridges Rust `bool` to Haskell `Bool` (True/False constructors).
impl FromHaskellSealed for bool {}
impl ToHaskellSealed for bool {}

impl FromHaskell for bool {
    fn from_value(value: &HaskellValue, table: &DataConTable) -> Result<Self, BridgeError> {
        match value {
            HaskellValue::Con(id, fields) => {
                let true_id = get_qualified(table, TRUE, 0)
                    .ok_or(BridgeError::UnknownDataConName("True".into()))?;
                let false_id = get_qualified(table, FALSE, 0)
                    .ok_or(BridgeError::UnknownDataConName("False".into()))?;

                if *id == true_id || *id == false_id {
                    if fields.is_empty() {
                        Ok(*id == true_id)
                    } else {
                        Err(BridgeError::ArityMismatch {
                            con: *id,
                            expected: 0,
                            got: fields.len(),
                        })
                    }
                } else {
                    Err(BridgeError::UnknownDataCon(*id))
                }
            }
            _ => Err(type_mismatch("Con", value)),
        }
    }
}

impl ToHaskell for bool {
    fn visit(
        &self,
        table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        let true_id = get_qualified(table, TRUE, 0)
            .ok_or_else(|| BridgeError::UnknownDataConName("True".into()))?;
        let false_id = get_qualified(table, FALSE, 0)
            .ok_or_else(|| BridgeError::UnknownDataConName("False".into()))?;
        visitor.begin_constructor(if *self { true_id } else { false_id }, 0)?;
        visitor.end_constructor()
    }
}

/// Also transparently unwraps `C#(c)` (boxed Char).
impl FromHaskellSealed for char {}
impl ToHaskellSealed for char {}

impl FromHaskell for char {
    fn from_value(value: &HaskellValue, table: &DataConTable) -> Result<Self, BridgeError> {
        let canonical_char = get_qualified(table, CHAR, 1);
        let canonical_text = get_qualified(table, TEXT, 3);
        let canonical_int = get_qualified(table, INT, 1);
        let decoded = match value {
            HaskellValue::Lit(Literal::LitChar(_) | Literal::LitWord(_)) => {
                shapes::unbox_char(value, table)
            }
            HaskellValue::Con(id, fields) if canonical_char == Some(*id) && fields.len() == 1 => {
                match &fields[0] {
                    literal @ HaskellValue::Lit(Literal::LitChar(_) | Literal::LitWord(_)) => {
                        shapes::unbox_char(literal, table)
                    }
                    HaskellValue::Con(text, fields)
                        if canonical_text == Some(*text) && fields.len() == 3 =>
                    {
                        shapes::text_bytes_checked_with(
                            fields,
                            |id| table.name_of(id) == Some("ByteArray"),
                            |id| canonical_int == Some(id),
                        )
                        .ok()
                        .filter(|bytes| bytes.len() == 1)
                        .map(|bytes| bytes[0] as char)
                    }
                    _ => None,
                }
            }
            _ => None,
        };
        decoded.ok_or_else(|| type_mismatch("LitChar or C#", value))
    }
}

impl ToHaskell for char {
    fn visit(
        &self,
        table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        let id = get_qualified(table, CHAR, 1)
            .ok_or_else(|| BridgeError::UnknownDataConName("C#".into()))?;
        visitor.begin_constructor(id, 1)?;
        visitor.literal(Literal::LitChar(*self))?;
        visitor.end_constructor()
    }
}

impl FromHaskellSealed for String {}
impl ToHaskellSealed for String {}

impl FromHaskell for String {
    fn from_value(value: &HaskellValue, table: &DataConTable) -> Result<Self, BridgeError> {
        let text_id = get_qualified(table, TEXT, 3);
        let nil_id = get_qualified(table, NIL, 0);
        let cons_id = get_qualified(table, CONS, 2);

        match value {
            // Text constructor: Text ByteArray# off len
            HaskellValue::Con(id, fields) if fields.len() == 3 && text_id == Some(*id) => {
                let int_id = get_qualified(table, INT, 1);
                let bytes = shapes::text_bytes_checked_with(
                    fields,
                    |id| table.name_of(id) == Some("ByteArray"),
                    |id| int_id == Some(id),
                )
                .map_err(|e| match e {
                    shapes::TextShapeError::WrongShape => {
                        type_mismatch("Text ByteArray# Int# Int#", value)
                    }
                    shapes::TextShapeError::BadBacking => match &fields[0] {
                        HaskellValue::Con(other_id, _) => {
                            let name = table.name_of(*other_id).unwrap_or("<unknown>");
                            BridgeError::TypeMismatch {
                                expected: "ByteArray or ByteArray# in Text".to_string(),
                                got: format!("Con({})", name),
                            }
                        }
                        other => type_mismatch("ByteArray or ByteArray# in Text", other),
                    },
                    shapes::TextShapeError::BadOffLen => type_mismatch("LitInt or I#", &fields[1]),
                    shapes::TextShapeError::BadSlice { off, len, ba_len } => {
                        BridgeError::TypeMismatch {
                            expected: "valid Text slice (non-negative off/len, in bounds)"
                                .to_string(),
                            got: format!("off={}, len={}, ba_len={}", off, len, ba_len),
                        }
                    }
                })?;
                String::from_utf8(bytes).map_err(|e| BridgeError::TypeMismatch {
                    expected: "UTF-8 Text".to_string(),
                    got: format!("Invalid UTF-8: {}", e),
                })
            }
            HaskellValue::Lit(Literal::LitString(bytes)) => String::from_utf8(bytes.clone())
                .map_err(|e| BridgeError::TypeMismatch {
                    expected: "UTF-8 String".to_string(),
                    got: format!("Invalid UTF-8: {}", e),
                }),
            // Also accept cons-cell list of Char (from ++ desugaring)
            HaskellValue::Con(_, _) => {
                let mut chars = Vec::new();
                let mut cur = value;
                loop {
                    match cur {
                        HaskellValue::Con(tag, fields)
                            if nil_id == Some(*tag) && fields.is_empty() =>
                        {
                            break;
                        }
                        HaskellValue::Con(tag, fields)
                            if cons_id == Some(*tag) && fields.len() == 2 =>
                        {
                            chars.push(char::from_value(&fields[0], table)?);
                            cur = &fields[1];
                        }
                        _ => return Err(type_mismatch("[] or (:)", cur)),
                    }
                }
                Ok(chars.into_iter().collect())
            }
            _ => Err(type_mismatch("Text, LitString, or [Char]", value)),
        }
    }
}

impl ToHaskell for String {
    fn visit(
        &self,
        table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        let text_id = get_qualified(table, TEXT, 3)
            .ok_or_else(|| BridgeError::UnknownDataConName("Data.Text.Text".into()))?;
        visitor.begin_constructor(text_id, 3)?;
        visitor.byte_array(self.as_bytes().to_vec())?;
        visitor.literal(Literal::LitInt(0))?;
        visitor.literal(Literal::LitInt(self.len() as i64))?;
        visitor.end_constructor()
    }
}

// Containers

impl<T> FromHaskellSealed for Option<T> {}
impl<T> ToHaskellSealed for Option<T> {}

impl<T: FromHaskell> FromHaskell for Option<T> {
    fn from_value(value: &HaskellValue, table: &DataConTable) -> Result<Self, BridgeError> {
        let nothing_id = get_qualified(table, NOTHING, 0);
        let just_id = get_qualified(table, JUST, 1);

        match value {
            HaskellValue::Con(id, fields) => {
                if nothing_id == Some(*id) {
                    if fields.is_empty() {
                        Ok(None)
                    } else {
                        Err(BridgeError::ArityMismatch {
                            con: *id,
                            expected: 0,
                            got: fields.len(),
                        })
                    }
                } else if just_id == Some(*id) {
                    if fields.len() == 1 {
                        Ok(Some(T::from_value(&fields[0], table).map_err(
                            |source| field_decode_error("Just", 1, &fields[0], table, source),
                        )?))
                    } else {
                        Err(BridgeError::ArityMismatch {
                            con: *id,
                            expected: 1,
                            got: fields.len(),
                        })
                    }
                } else {
                    Err(BridgeError::UnknownDataCon(*id))
                }
            }
            _ => Err(type_mismatch("Con", value)),
        }
    }
}

impl<T: ToHaskell> ToHaskell for Option<T> {
    fn visit(
        &self,
        table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        match self {
            None => {
                let id = get_qualified(table, NOTHING, 0)
                    .ok_or_else(|| BridgeError::UnknownDataConName("Nothing".into()))?;
                visitor.begin_constructor(id, 0)?;
                visitor.end_constructor()
            }
            Some(x) => {
                let id = get_qualified(table, JUST, 1)
                    .ok_or_else(|| BridgeError::UnknownDataConName("Just".into()))?;
                visitor.begin_constructor(id, 1)?;
                x.visit(table, visitor)?;
                visitor.end_constructor()
            }
        }
    }
}

impl<T> FromHaskellSealed for Vec<T> {}
impl<T> ToHaskellSealed for Vec<T> {}

impl<T: FromHaskell> FromHaskell for Vec<T> {
    fn from_value(value: &HaskellValue, table: &DataConTable) -> Result<Self, BridgeError> {
        let nil_id =
            get_qualified(table, NIL, 0).ok_or(BridgeError::UnknownDataConName("[]".into()))?;
        let cons_id =
            get_qualified(table, CONS, 2).ok_or(BridgeError::UnknownDataConName(":".into()))?;

        let mut res = Vec::new();
        let mut curr = value;

        loop {
            match curr {
                HaskellValue::Con(id, fields) => {
                    if *id == nil_id {
                        if fields.is_empty() {
                            break;
                        } else {
                            return Err(BridgeError::ArityMismatch {
                                con: *id,
                                expected: 0,
                                got: fields.len(),
                            });
                        }
                    } else if *id == cons_id {
                        if fields.len() == 2 {
                            res.push(T::from_value(&fields[0], table)?);
                            curr = &fields[1];
                        } else {
                            return Err(BridgeError::ArityMismatch {
                                con: *id,
                                expected: 2,
                                got: fields.len(),
                            });
                        }
                    } else {
                        return Err(BridgeError::UnknownDataCon(*id));
                    }
                }
                _ => return Err(type_mismatch("Con", curr)),
            }
        }

        Ok(res)
    }
}

impl<T: ToHaskell> ToHaskell for Vec<T> {
    fn visit(
        &self,
        table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        let nil_id = get_qualified(table, NIL, 0)
            .ok_or_else(|| BridgeError::UnknownDataConName("[]".into()))?;
        let cons_id = get_qualified(table, CONS, 2)
            .ok_or_else(|| BridgeError::UnknownDataConName(":".into()))?;

        for x in self {
            visitor.begin_constructor(cons_id, 2)?;
            x.visit(table, visitor)?;
        }
        visitor.begin_constructor(nil_id, 0)?;
        visitor.end_constructor()?;
        for _ in self {
            visitor.end_constructor()?;
        }
        Ok(())
    }
}

impl<T, E> FromHaskellSealed for Result<T, E> {}
impl<T, E> ToHaskellSealed for Result<T, E> {}

impl<T: FromHaskell, E: FromHaskell> FromHaskell for Result<T, E> {
    fn from_value(value: &HaskellValue, table: &DataConTable) -> Result<Self, BridgeError> {
        let right_id = get_qualified(table, RIGHT, 1);
        let left_id = get_qualified(table, LEFT, 1);

        match value {
            HaskellValue::Con(id, fields) => {
                if right_id == Some(*id) {
                    if fields.len() == 1 {
                        Ok(Ok(T::from_value(&fields[0], table)?))
                    } else {
                        Err(BridgeError::ArityMismatch {
                            con: *id,
                            expected: 1,
                            got: fields.len(),
                        })
                    }
                } else if left_id == Some(*id) {
                    if fields.len() == 1 {
                        Ok(Err(E::from_value(&fields[0], table)?))
                    } else {
                        Err(BridgeError::ArityMismatch {
                            con: *id,
                            expected: 1,
                            got: fields.len(),
                        })
                    }
                } else {
                    Err(BridgeError::UnknownDataCon(*id))
                }
            }
            _ => Err(type_mismatch("Con", value)),
        }
    }
}

impl<T: ToHaskell, E: ToHaskell> ToHaskell for Result<T, E> {
    fn visit(
        &self,
        table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        let (id, value): (DataConId, &dyn ToHaskell) = match self {
            Ok(x) => {
                let id = get_qualified(table, RIGHT, 1)
                    .ok_or_else(|| BridgeError::UnknownDataConName("Right".into()))?;
                (id, x)
            }
            Err(e) => {
                let id = get_qualified(table, LEFT, 1)
                    .ok_or_else(|| BridgeError::UnknownDataConName("Left".into()))?;
                (id, e)
            }
        };
        visitor.begin_constructor(id, 1)?;
        value.visit(table, visitor)?;
        visitor.end_constructor()
    }
}

// Tuples

impl<A, B> FromHaskellSealed for (A, B) {}
impl<A, B> ToHaskellSealed for (A, B) {}

impl<A: FromHaskell, B: FromHaskell> FromHaskell for (A, B) {
    fn from_value(value: &HaskellValue, table: &DataConTable) -> Result<Self, BridgeError> {
        // Missing-constructor is a DIFFERENT failure than "this value isn't a
        // pair": the old `pair_id == Some(*id)` fell through to the same
        // `TypeMismatch("(,)", ...)` in both cases, masking a table that
        // simply never registered `(,)` (#F6).
        let pair_id = get_qualified(table, PAIR, 2)
            .ok_or_else(|| BridgeError::UnknownDataConName("(,)".into()))?;
        match value {
            HaskellValue::Con(id, fields) if *id == pair_id => {
                if fields.len() == 2 {
                    Ok((
                        A::from_value(&fields[0], table)?,
                        B::from_value(&fields[1], table)?,
                    ))
                } else {
                    Err(BridgeError::ArityMismatch {
                        con: *id,
                        expected: 2,
                        got: fields.len(),
                    })
                }
            }
            _ => Err(type_mismatch("(,)", value)),
        }
    }
}

impl<A: ToHaskell, B: ToHaskell> ToHaskell for (A, B) {
    fn visit(
        &self,
        table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        let pair_id = get_qualified(table, PAIR, 2)
            .ok_or_else(|| BridgeError::UnknownDataConName("(,)".into()))?;
        visitor.begin_constructor(pair_id, 2)?;
        self.0.visit(table, visitor)?;
        self.1.visit(table, visitor)?;
        visitor.end_constructor()
    }
}

impl<A, B, C> FromHaskellSealed for (A, B, C) {}
impl<A, B, C> ToHaskellSealed for (A, B, C) {}

impl<A: FromHaskell, B: FromHaskell, C: FromHaskell> FromHaskell for (A, B, C) {
    fn from_value(value: &HaskellValue, table: &DataConTable) -> Result<Self, BridgeError> {
        // See the 2-tuple impl above: missing-constructor is split out of the
        // type-mismatch arm (#F6).
        let triple_id = get_qualified(table, TRIPLE, 3)
            .ok_or_else(|| BridgeError::UnknownDataConName("(,,)".into()))?;
        match value {
            HaskellValue::Con(id, fields) if *id == triple_id => {
                if fields.len() == 3 {
                    Ok((
                        A::from_value(&fields[0], table)?,
                        B::from_value(&fields[1], table)?,
                        C::from_value(&fields[2], table)?,
                    ))
                } else {
                    Err(BridgeError::ArityMismatch {
                        con: *id,
                        expected: 3,
                        got: fields.len(),
                    })
                }
            }
            _ => Err(type_mismatch("(,,)", value)),
        }
    }
}

impl<A: ToHaskell, B: ToHaskell, C: ToHaskell> ToHaskell for (A, B, C) {
    fn visit(
        &self,
        table: &DataConTable,
        visitor: &mut dyn HaskellVisitor,
    ) -> Result<(), BridgeError> {
        let triple_id = get_qualified(table, TRIPLE, 3)
            .ok_or_else(|| BridgeError::UnknownDataConName("(,,)".into()))?;
        visitor.begin_constructor(triple_id, 3)?;
        self.0.visit(table, visitor)?;
        self.1.visit(table, visitor)?;
        self.2.visit(table, visitor)?;
        visitor.end_constructor()
    }
}

#[cfg(test)]
#[allow(clippy::approx_constant)] // tests use 3.14159 as a float round-trip literal
mod tests {
    use super::*;
    use tidepool_repr::{DataCon, DataConId};

    fn roundtrip<T: FromHaskell + ToHaskell + PartialEq + std::fmt::Debug>(
        value: T,
        table: &DataConTable,
    ) {
        let encoded = value.to_value(table).expect("ToHaskell failed");
        let decoded = T::from_value(&encoded, table).expect("FromHaskell failed");
        assert_eq!(value, decoded, "roundtrip failed");
    }

    fn test_table() -> DataConTable {
        let mut t = DataConTable::new();
        // Nothing=0, Just=1, False=2, True=3, ()=4, Nil=5, Cons=6, (,,)=7, Right=8, Left=9
        // I#=10, W#=11, D#=12, C#=13
        t.insert(DataCon {
            id: DataConId(0),
            name: "Nothing".into(),
            tag: 1,
            rep_arity: 0,
            field_bangs: vec![],
            qualified_name: Some("GHC.Maybe.Nothing".into()),
            type_name: String::new(),
        });
        t.insert(DataCon {
            id: DataConId(1),
            name: "Just".into(),
            tag: 2,
            rep_arity: 1,
            field_bangs: vec![],
            qualified_name: Some("GHC.Maybe.Just".into()),
            type_name: String::new(),
        });
        t.insert(DataCon {
            id: DataConId(2),
            name: "False".into(),
            tag: 1,
            rep_arity: 0,
            field_bangs: vec![],
            qualified_name: Some("GHC.Types.False".into()),
            type_name: String::new(),
        });
        t.insert(DataCon {
            id: DataConId(3),
            name: "True".into(),
            tag: 2,
            rep_arity: 0,
            field_bangs: vec![],
            qualified_name: Some("GHC.Types.True".into()),
            type_name: String::new(),
        });
        t.insert(DataCon {
            id: DataConId(4),
            name: "(,)".into(),
            tag: 1,
            rep_arity: 2,
            field_bangs: vec![],
            qualified_name: Some("GHC.Tuple.(,)".into()),
            type_name: String::new(),
        });
        t.insert(DataCon {
            id: DataConId(5),
            name: "[]".into(),
            tag: 1,
            rep_arity: 0,
            field_bangs: vec![],
            qualified_name: Some("GHC.Types.[]".into()),
            type_name: String::new(),
        });
        t.insert(DataCon {
            id: DataConId(6),
            name: ":".into(),
            tag: 2,
            rep_arity: 2,
            field_bangs: vec![],
            qualified_name: Some("GHC.Types.:".into()),
            type_name: String::new(),
        });
        t.insert(DataCon {
            id: DataConId(7),
            name: "(,,)".into(),
            tag: 1,
            rep_arity: 3,
            field_bangs: vec![],
            qualified_name: Some("GHC.Tuple.(,,)".into()),
            type_name: String::new(),
        });
        t.insert(DataCon {
            id: DataConId(8),
            name: "Right".into(),
            tag: 2,
            rep_arity: 1,
            field_bangs: vec![],
            qualified_name: Some("Data.Either.Right".into()),
            type_name: String::new(),
        });
        t.insert(DataCon {
            id: DataConId(9),
            name: "Left".into(),
            tag: 1,
            rep_arity: 1,
            field_bangs: vec![],
            qualified_name: Some("Data.Either.Left".into()),
            type_name: String::new(),
        });
        t.insert(DataCon {
            id: DataConId(10),
            name: "I#".into(),
            tag: 1,
            rep_arity: 1,
            field_bangs: vec![],
            qualified_name: Some("GHC.Types.I#".into()),
            type_name: String::new(),
        });
        t.insert(DataCon {
            id: DataConId(11),
            name: "W#".into(),
            tag: 1,
            rep_arity: 1,
            field_bangs: vec![],
            qualified_name: Some("GHC.Types.W#".into()),
            type_name: String::new(),
        });
        t.insert(DataCon {
            id: DataConId(12),
            name: "D#".into(),
            tag: 1,
            rep_arity: 1,
            field_bangs: vec![],
            qualified_name: Some("GHC.Types.D#".into()),
            type_name: String::new(),
        });
        t.insert(DataCon {
            id: DataConId(13),
            name: "C#".into(),
            tag: 1,
            rep_arity: 1,
            field_bangs: vec![],
            qualified_name: Some("GHC.Types.C#".into()),
            type_name: String::new(),
        });
        t.insert(DataCon {
            id: DataConId(14),
            name: "()".into(),
            tag: 1,
            rep_arity: 0,
            field_bangs: vec![],
            qualified_name: Some("GHC.Tuple.()".into()),
            type_name: String::new(),
        });
        t.insert(DataCon {
            id: DataConId(15),
            name: "Text".into(),
            tag: 1,
            rep_arity: 3,
            field_bangs: vec![],
            qualified_name: Some("Data.Text.Text".into()),
            type_name: String::new(),
        });
        t
    }

    #[test]
    fn test_i32_roundtrip() {
        let table = test_table();
        roundtrip(42i32, &table);
        roundtrip(-7i32, &table);
    }

    #[test]
    fn test_i32_overflow() {
        let table = test_table();
        let val: i64 = i32::MAX as i64 + 1;
        let value = val.to_value(&table).unwrap();
        let res = i32::from_value(&value, &table);
        assert!(matches!(res, Err(BridgeError::TypeMismatch { .. })));
    }

    #[test]
    fn test_nested_roundtrip() {
        let table = test_table();
        roundtrip(vec![Some(1i64), None, Some(3)], &table);
        roundtrip(Some((42i64, vec![true, false])), &table);
    }

    #[test]
    fn test_unknown_datacon() {
        let table = test_table();
        let value = HaskellValue::Con(DataConId(100), vec![]);
        let res = bool::from_value(&value, &table);
        assert!(matches!(
            res,
            Err(BridgeError::UnknownDataCon(DataConId(100)))
        ));
    }

    #[test]
    fn test_arity_mismatch() {
        let table = test_table();
        let true_id = table.get_by_name("True").unwrap();
        let value = HaskellValue::Con(true_id, vec![HaskellValue::Lit(Literal::LitInt(1))]);
        let res = bool::from_value(&value, &table);
        assert!(matches!(res, Err(BridgeError::ArityMismatch { .. })));
    }

    #[test]
    fn test_type_mismatch() {
        let table = test_table();
        let value = HaskellValue::Lit(Literal::LitInt(1));
        let res = bool::from_value(&value, &table);
        assert!(matches!(res, Err(BridgeError::TypeMismatch { .. })));
    }

    #[test]
    fn test_unit_roundtrip() {
        let table = test_table();
        roundtrip((), &table);
    }

    #[test]
    fn test_vec_string_roundtrip() {
        let table = test_table();
        roundtrip(vec!["a".to_string(), "b".to_string()], &table);
    }

    #[test]
    fn test_option_nested_roundtrip() {
        let table = test_table();
        roundtrip(Some(vec![1i64, 2]), &table);
        roundtrip(None::<Vec<i64>>, &table);
    }

    #[test]
    fn test_result_nested_roundtrip() {
        let table = test_table();
        roundtrip(Ok::<Vec<i64>, String>(vec![1, 2]), &table);
        roundtrip(Err::<Vec<i64>, String>("error".to_string()), &table);
    }

    // === F6/F7 round-trip asymmetry regression tests ===

    /// `to_value`/`from_value` for `()` must be symmetric: only the `()`
    /// constructor decodes to unit. Before the fix, `from_value` accepted ANY
    /// nullary constructor (`False` here), silently masking an upstream
    /// encoding bug that produced the wrong nullary con.
    #[test]
    fn unit_from_value_rejects_other_nullary_constructors() {
        let table = test_table();
        let false_id = table.get_by_name("False").unwrap();
        let wrong_nullary = HaskellValue::Con(false_id, vec![]);
        let result = <()>::from_value(&wrong_nullary, &table);
        assert!(
            result.is_err(),
            "from_value(()) must reject a non-`()` nullary constructor (False), got Ok"
        );
    }

    /// A `DataConTable` missing `(,)` entirely must report `UnknownDataConName`
    /// from `from_value`, not the same `TypeMismatch` a genuinely-wrong-shaped
    /// value gets — the two failures have different fixes (register the
    /// constructor vs. fix the caller's data).
    #[test]
    fn pair_from_value_missing_constructor_is_unknown_dataconname_not_type_mismatch() {
        let table = DataConTable::new(); // no "(,)" registered
        let some_other_con = HaskellValue::Con(DataConId(0), vec![]);
        let err = <(i64, i64)>::from_value(&some_other_con, &table).unwrap_err();
        assert!(
            matches!(err, BridgeError::UnknownDataConName(ref n) if n == "(,)"),
            "expected UnknownDataConName(\"(,)\"), got {err:?}"
        );
    }

    /// With `(,)` registered, a value that just isn't a pair still reports the
    /// ORIGINAL type-mismatch shape (only the missing-constructor case moved).
    #[test]
    fn pair_from_value_wrong_shape_is_still_type_mismatch() {
        let table = test_table();
        let not_a_pair = HaskellValue::Con(table.get_by_name("Nothing").unwrap(), vec![]);
        let err = <(i64, i64)>::from_value(&not_a_pair, &table).unwrap_err();
        assert!(
            matches!(err, BridgeError::TypeMismatch { .. }),
            "expected TypeMismatch, got {err:?}"
        );
    }

    #[test]
    fn triple_from_value_missing_constructor_is_unknown_dataconname() {
        let table = DataConTable::new(); // no "(,,)" registered
        let some_other_con = HaskellValue::Con(DataConId(0), vec![]);
        let err = <(i64, i64, i64)>::from_value(&some_other_con, &table).unwrap_err();
        assert!(
            matches!(err, BridgeError::UnknownDataConName(ref n) if n == "(,,)"),
            "expected UnknownDataConName(\"(,,)\"), got {err:?}"
        );
    }
}
