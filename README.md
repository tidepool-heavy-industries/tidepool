...
The `Llm` effect handles classification, extraction, and judgment requests. It supports multiple backends:

- **anthropic** (default) — uses the [genai](https://github.com/jeremychone/rust-genai) crate (supports Anthropic, Google, Ollama). Requires `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`, etc.
- **openai** — a lightweight `ureq`-based client for OpenAI-compatible APIs. Enabled via `TIDEPOOL_LLM_PROVIDER=openai`.

**Environment variables:**
- `TIDEPOOL_LLM_PROVIDER` — `anthropic` or `openai` (default: `anthropic`)
- `TIDEPOOL_LLM_MODEL` — model name override (OpenAI default: `gpt-4o-mini`; Anthropic default: `ollama:llama3.2` via `genai`)
- `OPENAI_API_KEY` — required for the `openai` provider
- `OPENAI_BASE_URL` — optional base URL for the `openai` provider (default: `https://api.openai.com/v1`)

> **Structured Outputs**: OpenAI-compatible backends use `response_format: { type: "json_schema" }` with `strict: false` to accommodate optional schema fields generated by the Haskell `schemaToValue` helper.
