...
async fn selfharness_spine_one_cycle_render_loop_finalize_render() {
    ...
    // The nested Agent: the SAME `EngineConfig::standard` every Agent turn
    // uses, with `examples/harness` as its project_lib so an answerer's
    // `finalize @Decision (...)` can `import Harness (Decision(..),
    // Confidence(..))` from the reference harness module directly.
