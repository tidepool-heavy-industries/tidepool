...
//! declares (`bridge/mcp/src/effect_defs.rs`), rather than being generated
//! from a shared Rust struct. Each pair MUST stay in sync; the
//! `stable_records` test (`bridge/handlers/tests/bridged_records.rs`) pins
...
/// `bridge/haskell/lib/Tidepool/Records/Stable.hs` (kept in sync by the
...
pub fn stable_records_module() -> String {
    ...
    out.push_str("-- Tidepool.Prelude (see bridge/mcp/src/\n");
