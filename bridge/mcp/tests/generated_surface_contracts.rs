...
/// `bridge/haskell/lib/Tidepool/Worktree.hs` and must NOT also be emitted here. Two
    ...
    for name in [
        ...
        assert!(
            ...
            "generated Tidepool.Effects still defines `{name}`, which now lives in \
             bridge/haskell/lib/Tidepool/Worktree.hs — two definitions in one row collide"
        );
        assert!(
            authored_worktree_module().contains(name),
            "bridge/haskell/lib/Tidepool/Worktree.hs no longer defines `{name}`, and the \
...
/// `bridge/haskell/lib/Tidepool/Event.hs` and must NOT also be emitted here — same
...
fn generated_module_does_not_redefine_the_relocated_event_helpers() {
    ...
    for name in [
        ...
        assert!(
            !src.contains(name),
            "generated Tidepool.Effects still defines `{name}`, which now lives in \
             bridge/haskell/lib/Tidepool/Event.hs — two definitions in one row collide"
        );
        assert!(
            authored_event_module().contains(name),
            "bridge/haskell/lib/Tidepool/Event.hs no longer defines `{name}`, and the generated \
