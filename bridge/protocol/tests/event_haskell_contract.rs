...
//! relocate too) — see `bridge/protocol/src/effects/event.rs`'s module doc.
//!
//! These literal strings are HAND-TRANSCRIBED from `event_effect_def!`
//! (`bridge/mcp/src/effect_defs.rs`) as it stood before the migration flip —
...
/// `bridge/protocol/src/effects/event.rs`. Cross-checked here against the
/// schema's own helper list so a future edit that accidentally makes one of
/// these representable (and forgets to delete its `bridge/haskell/lib/Tidepool/
    ...
    for name in NOT_REPRESENTABLE {
        assert!(
            ...
            "{name} was expected to stay non-representable (relocated to \
             bridge/haskell/lib/Tidepool/Event.hs), but the schema now describes it — \
