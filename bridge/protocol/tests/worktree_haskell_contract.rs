...
//!
//! `bridge/protocol/src/effects/worktree.rs` describes the Worktree effect
...
fn worktree_remaining_decl_fields_are_pinned() {
    ...
    // surface: they are now DEFINED in `bridge/haskell/lib/Tidepool/Worktree.hs`, and
    // a row containing Worktree imports that module. Matches
    // `extra_imports_for!(Worktree)` in `bridge/mcp/src/effect_defs.rs`
