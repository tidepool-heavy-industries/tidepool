...
//! `bridge/haskell/lib/Tidepool/Event.hs` alongside the eighteen non-representable
...
//! are DEFINITIONS in `bridge/haskell/lib/Tidepool/Event.hs`, reached through this
...
//! (`bridge/handlers/src/handlers/event.rs`) uses the `Ev*` wire structs
//! directly as its working representation. `has_adapters` stays false, and no
//! `bridge/handlers/src/generated/event_adapters.rs` is emitted.
//!
...
use crate::schema::{
    ...
    IdentityPayload, JsonInstance, OuterEffect, Polymorphism, RecordField, RustBinding, SumVariant,
...
/// `bridge/haskell/lib/Tidepool/Event.hs` as DEFINITIONS — the per-helper verdict is
