...
fn ensure_memory_store(store: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    std::fs::write(
        ...
        prompt_catalog::PromptId::MemoryCurator.body(),
    )?;
...
/// The durable data root (not the regenerable cache): `$XDG_DATA_HOME`, or
/// `$HOME/.local/share` when unset.
