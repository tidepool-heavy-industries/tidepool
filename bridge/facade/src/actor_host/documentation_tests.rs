...
async fn floating_point_resident_display_matches_prelude() {
    tokio::time::timeout(Duration::from_secs(120), async {
        ...
        let root = campaign.root_installation.policy.clone();
        let setup =
            dispatch_haskell_script(root.as_ref(), include_str!("numeric_display_fixture.hs"))
                .await;
        ...
        let probes = [
            ...
            ("inspectFull d", "1.0"),
            ("inspectFull f", "1.0"),
            // Structural Display separates fields with newlines so a retained
            // tree can page between them without evaluating the next field.
            ("(d, f)", "(1.0,\n1.0)"),
            ("[d, d]", "[1.0,\n1.0]"),
            ("[f, f]", "[1.0,\n1.0]"),
            ("NumericRecord d f", "NumericRecord 1.0,\n1.0"),
            ("inspectFull (NumericRecord d f)", "NumericRecord 1.0,\n1.0"),
            // A displayed String is literal text; these still assert Prelude's
            // Show contents without adding another layer of String quotation.
            ("P.show (d, f)", "(1.0,1.0)"),
            ("P.show [d, d]", "[1.0,1.0]"),
            ("P.show [f, f]", "[1.0,1.0]"),
            ("P.show (NumericRecord d f)", "NumericRecord 1.0 1.0"),
            // Tidepool.Prelude.show is likewise String-valued and displays as text.
            ("show d", "1.0"),
            ("show f", "1.0"),
            (
    ...
    .await
    .expect("floating-point display probe timed out");
}
