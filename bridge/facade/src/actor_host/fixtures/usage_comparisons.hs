...
let unknownRow = base { rosterActorId = 104, rosterProviderThread = Nothing, rosterUsageSummary = Nothing }
let pendingRow = base { rosterActorId = 105, rosterProviderThread = Just "pending", rosterUsageSummary = Nothing }
let turnOnlyRow = base { rosterActorId = 106, rosterProviderThread = Just "thread", rosterUsageSummary = Just (ProviderUsageSummary (UsageTurn "thread" "turn") UsageComplete 1 10 0 0 0 10) }
let emptyUsage = swarmUsage (SwarmSnapshot [unknownRow, pendingRow, turnOnlyRow])
let deltaUnknownUsage = comparableUsage (usageDelta (SwarmSnapshot []) (SwarmSnapshot [unknownRow]))
...
inspectFull (and checks && partialProviderThreads (comparableUsage partialChange) == ["thread"] && totalTokens (comparableUsage partialChange) == 20 && length (unknownActors (swarmUsage (SwarmSnapshot [unknownRow]))) == 1 && totalTokens (newlyObservedUsage (usageDelta (SwarmSnapshot [unknownRow]) (SwarmSnapshot [newer]))) == 30 && null (observedProviderThreads emptyUsage) && totalTokens emptyUsage == 0 && map snd (unobservedProviderActors emptyUsage) == [NoProviderThread, ThreadUsageNotObserved, TurnUsageOnly] && unobservedProviderActors deltaUnknownUsage == [((104, rosterActorIncarnation unknownRow), NoProviderThread)])
