...
let collector = R.definition "command-completions" Actor.ReadOnly Completions { seen = 0, done = R.on (Cmd.completion job) (\_ -> print ("completion observed" :: Text) >> modify' (+1)), completionCount = \() -> get }
