...
pub fn classify_error(text: &str) -> Classification {
    ...
    // Tidepool's own generated-wrapper location note — never a raw GHC
    // message, so check it before anything GHC-shaped. Retain the old wording
    // because persisted compile reports may predate the diagnostic correction.
    if text.contains("only on generated workbench wrapper lines")
        || text.contains("arose in the harness's result-display wrapper")
    {
...
mod tests {
    ...
    // message quoted verbatim in bridge/haskell/lib/Tidepool/Form/Check.hs:109
    ...
    // harness's own generated-location note, emitted by
    // `tidepool/toolchain/src/diag.rs`.

    #[test]
    fn generated_wrapper_location_is_wrapper_origin() {
        let c = classify_error(
            "GHC error (1 diagnostic(s)):\nGHC reported 1 error(s) only on generated workbench wrapper lines. Those locations do not identify whether the root cause is in the authored input or generated framing.",
        );
        assert_eq!(c.bucket, DesireBucket::WrapperOrigin);
    }

    #[test]
    fn legacy_wrapper_wording_remains_classified_for_persisted_reports() {
        let c = classify_error(
