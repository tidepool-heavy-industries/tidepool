...
use exomonad_node::ProcessMountBoundary;
...
fn executable_enters_retained_view_without_runtime_or_mount_privileges() {
    ...
    let run = |entry: &str, script: &str| {
        Command::new(env!("CARGO_BIN_EXE_exomonad"))
...
fn executable_rejects_uncontained_payload_before_execution() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_exomonad"))
        .args([
            "in-slice",
            "--slice",
            "exomonad-unconfigured-test.slice",
            "--",
            "/bin/sh",
            "-c",
            "touch \"$1\"",
...
async fn executable_enters_slice_through_outside_tmux_server() {
    use exomonad_node::systemd_slice::SystemdSlice;
    use exomonad_node::{ProcessInvocation, TmuxLaunch, TmuxSession};
    ...
    let storage = tempfile::tempdir().unwrap();
    let socket = format!("exomonad-slice-test-{}", uuid::Uuid::new_v4().simple());
    ...
    let seed = tmux
        ...
        .await
        .unwrap();
    let executable = std::path::Path::new(env!("CARGO_BIN_EXE_exomonad"));
