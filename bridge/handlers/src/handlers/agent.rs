...
enum Cycle {
    ...
    /// Haskell (`exomonad/agent/CLAUDE.md`, "the seam is a STEP function").
...
impl AsyncCycle {
    ...
    ///
    /// Order is the rule from `exomonad/agent/CLAUDE.md`: settling before the
...
impl SubagentHandler {
    ...
    /// (`exomonad/worktree/CLAUDE.md`). Fallible: opening the registry and the
    ...
    pub fn with_backends(
        ...
        // (`exomonad/agent/src/spawn.rs`'s `cwd`/`git_dir`). Decode ONCE here,
...
/// `exomonad/agent/CLAUDE.md` forbids a backend that owns a process from
...
/// Total map from the domain `SpawnError` (`exomonad/agent/src/spawn.rs`) to
...
mod tests {
    ...
    // never a mock of git, exomonad/worktree/CLAUDE.md) with the registry,
    ...
    /// (`exomonad/agent/src/spawn.rs`) — a non-UTF-8 root must be rejected
