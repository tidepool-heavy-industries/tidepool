...
startComponent = do
  ...
  void $ turn owner ("let baseline = GitRef " <> literal baseline)
...
workbench = do
  ...
  pending <- turn (checkActor lead) "pollReply sessionReply"
  ...
  delivered <- turn owner "delivered <- pollResponse (forkedResponse lead)\ninspectFull delivered"
...
checkImprovement owner = do
  ...
  void $ turn (checkActor improver) ("respond (Produced (Candidate " <> literal next <> " [\"authored guidance check\"] [\"activate next swarm\"]))")
