...
let boxDefinition :: ActorSpec Box BoxEffects
    boxDefinition = R.definition "box" (Actor.Selected knownEffects) Box { boxState = BoxState 0, boxView = \() -> get, boxRun = \program -> do { modify' (\s -> s { boxCalls = boxCalls s + 1 }); pure (T.length program) } }
...
let callerDefinition :: ActorSpec Caller CallerEffects
    callerDefinition = R.definition "caller" (Actor.Selected knownEffects) Caller { callerState = CallerState [], callerView = \() -> get, callerAsk = \(box, program) -> do { code <- R.call (boxRun (R.client box)) program; modify' (\s -> s { callerReplies = callerReplies s ++ [code] }) } }
