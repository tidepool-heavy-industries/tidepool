...
# GHC oracle sidecar: the GATING referee for the real-Core corpus differential
# (tidepool-codegen/tests/real_core_corpus.rs). Generated by plain-GHC
# execution of Corpus.hs itself (via `corpus-oracle-gen`, a test-suite stanza
# in tidepool-extract.cabal) — unrelated to the extractor pipeline above, so
# it needs no TIDEPOOL_EXTRACT/TIDEPOOL_GHC_LIBDIR, just `cabal run`. One
# regen entry point produces both artifacts from the same Corpus.hs.
ORACLE="$OUT/oracle.json"
(cd "$HERE" && cabal run corpus-oracle-gen -- "$ORACLE")
echo "Generated GHC oracle sidecar: $ORACLE"

# oracle.json is NOT gitignored (only *.cbor is) — a plain add suffices.
git -C "$HERE/.." add "$ORACLE"
echo "git add'd corpus_cbor/oracle.json."
