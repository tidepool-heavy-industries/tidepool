...
module Tidepool.PreparedJson
  ...
  , classifyJson, resolveJsonAuthority, jsonValueLayoutForType
  ) where
...
classifyJson (JsonAuthority owner textUnit) binder
 ...
 where
  ...
  isValue ty = case splitTyConApp_maybe ty of
    ...
    _ -> False
  jsonLayoutForValue = jsonValueLayoutForType (JsonAuthority owner textUnit)
...
jsonValueLayoutForType (JsonAuthority owner textUnit) valueType = do
