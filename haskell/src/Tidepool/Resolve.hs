...
resolveExternals varIdFn hscEnv binds = do
  ...
  where
    ...
    isResolvable localSet v =
      ...
      && not (isSessionValVar v)

    -- | A reference to a tidepool-repl SESSION value binding
    -- (module @Tidepool.Session.Val.G<g>@). These are a DISTINCT resolution
    -- category (kimi B1/B2): their VALUE lives in the persistent JIT machine's
    -- heap, reached at codegen via the @ExternalEnv@ override keyed on the
    -- binder's @stableVarId@. Their interface is deliberately THIN (type only,
    -- no unfolding / no @mi_extra_decls@), so the front-end typechecker can see
    -- the type without GHC inlining the body.
    --
    -- We must NOT treat them as resolvable externals: resolving would (B1) try
    -- to inline a body that isn't there, and FAILING to resolve would (B2) drop
    -- them into @tsUnresolvedIds@ → Translate emits the @0x45@ error sentinel,
    -- discarding the session id so the @ExternalEnv@ override never fires.
    -- Excluding them here leaves the bare external @Var@ in place, which
