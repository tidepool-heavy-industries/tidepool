...
resolveExternals varIdFn hscEnv binds = do
  ...
  where
    ...
    -- category: their VALUE lives in the persistent JIT machine's
    ...
    --
    -- We must NOT treat them as resolvable externals: resolving would try
    -- to inline a body that isn't there, and failed resolution would drop
