...
module Tidepool.ExecutionSchema
  ...
  , AlternativePattern(..), Alternative(..), Expr(..), FieldLayout(..)
...
data GlobalDecl = GlobalDecl
  { globalIdentity :: SymbolIdentity, globalSignature :: SignatureId
...
data ScalarLiteral = IntLiteral Word8 ByteString | WordLiteral Word8 ByteString
  | FloatLiteral Word8 ByteString | CharLiteral Word32 | BytesLiteral ByteString
  deriving stock (Eq, Show, Generic)
data Atom = Ref ValueRef | Scalar ScalarLiteral | Void deriving stock (Eq, Show, Generic)
...
data HeapBinding = HeapBinding { heapBindingId :: ValueId, heapBindingRhs :: HeapRhs }
  deriving stock (Eq, Show, Generic)
data HeapRhs = Function SignatureId [ValueId] [ValueRef] Expr
...
data Expr = Return [Atom] | Enter Atom SignatureId | Call Atom SignatureId [Atom] | Operation OperationId [Atom]
  ...
  | Case Expr ValueId [RuntimeRep] [Alternative]
