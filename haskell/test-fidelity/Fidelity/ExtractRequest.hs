...
typedInspectionDecodes = case workerRequestFromArgv
  (workerArgv [Input "Expr.hs", InspectType "fmap", InspectOut "inspection.cbor"]) of
  Right (Just request) ->
    requestInspection request == Just (InspectTypeOf "fmap")
      && requestInspectOut request == Just "inspection.cbor"
  _ -> False

typedRequestDecodes :: Bool
typedRequestDecodes = case workerRequestFromArgv ["--worker-request-v4", payload] of
  Right (Just request) -> requestFiles request == ["x"] && requestBindGen request == Just 42
  _ -> False
  where
    payload = "5450524551303034020000000101000000780b2a00000000000000"

wrongVersionRejected :: Bool
wrongVersionRejected = all rejected ["5450524551303031", "5450524551303033"]
  where
    rejected magic = isLeft (workerRequestFromArgv ["--worker-request-v4", magic ++ "00000000"])

retiredFlagsRejected :: Bool
retiredFlagsRejected = all rejected ["--worker-request-v1", "--worker-request-v2", "--worker-request-v3"]
  where
    rejected flag = isLeft (workerRequestFromArgv [flag, validPayload])
    validPayload = "545052455130303400000000"

unknownTagRejected :: Bool
unknownTagRejected = isLeft (workerRequestFromArgv
  ["--worker-request-v4", "545052455130303401000000ff"])

retiredTagsRejected :: Bool
retiredTagsRejected = all retired [9, 10, 14]
  where
    retired tag = workerRequestFromArgv
      ["--worker-request-v4", "545052455130303401000000" ++ byteHex tag]
...
truncatedFieldRejected = isLeft (workerRequestFromArgv
  ["--worker-request-v4", "545052455130303401000000010500000078"])
