...
/// instead — see [`gen::harness_req_rs`]'s doc for why.
///
/// # Panics
...
pub fn harness_generated_files() -> Vec<GeneratedFile> {
    ...
    let mut out: Vec<GeneratedFile> = harness_only
        .iter()
        .map(|e| gen::harness_req_rs::file(e, "tidepool-harness"))
        .collect();
    out.push(gen::harness_req_rs::module_index(
        &harness_only,
        "tidepool-harness",
    ));
    out
...
/// [`gen::harness_req_rs`]'s doc for the layering argument.
///
/// # Panics
...
pub fn runtime_generated_files() -> Vec<GeneratedFile> {
    ...
    let mut out: Vec<GeneratedFile> = effects
        .iter()
        .map(|e| gen::harness_req_rs::file(e, "tidepool-runtime"))
        .collect();
    out.push(gen::harness_req_rs::module_index(
        &effects,
        "tidepool-runtime",
