...
Coverage depends on which revisions have been published. Whether you need to
do anything depends on how Nix was installed:
...
Coverage varies by revision: [getting started](getting-started.md) still
describes a from-source build as the default path, since not every commit has
published artifacts. Configuring the substituter above lets Nix use a cached
artifact whenever one exists for the revision you build, and falls back to a
