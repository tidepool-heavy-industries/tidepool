#!/usr/bin/env python3
"""Actual Exomonad bootstrap/placement acceptance. Private offline Codex homes; no inference.

Use --exomonad for a packaged runner in a clean environment. Optional --codex,
...
def verify_wrong_service_slice(args, output, env):
    ...
    try:
        ...
        run(str(args.exomonad), "new", str(work), env=selected)
        config = work / ".exomonad/config.toml"
        config.write_text(config.read_text() + '\n[launch]\nsystemd_slice = "' + slice_name + '"\n')
        rejected = run(str(args.exomonad), "init", "--workspace", str(work),
...
def main():
    ...
    for key, value in (("TIDEPOOL_EXTRACT", args.extractor),
                       ...
                       ("EXOMONAD_INTERACTIVE_CODEX_BIN", args.codex)):
        if value is not None:
            env[key] = str(value.resolve())
    shared = None
    try:
        missing = output / "missing-slice"
        run(str(args.exomonad), "new", str(missing), env=env)
        config = missing / ".exomonad/config.toml"
        config.write_text(config.read_text() + '\n[launch]\nsystemd_slice = "exomonad-missing-' + uuid.uuid4().hex + '.slice"\n')
        rejected = run(str(args.exomonad), "init", "--workspace", str(missing),
...
''')
            ...
            started = run(str(args.exomonad), "init", "--workspace", str(work),
                          "--session", session, "--no-attach", env=selected, check=False)
            ...
            roots.append(root)
            units.add("exomonad-host-" + root.name + ".scope")
        ...
        for unit in units:
            run("systemctl", "--user", "stop", unit, check=False, timeout=30)
        run("systemctl", "--user", "stop", "exomonad-command-resources.service", check=False, timeout=30)
        # The fixture was the exclusive service owner. Remove only its stopped socket.
        if "MainPID=0\n" in service():
            socket = Path(os.environ["XDG_RUNTIME_DIR"]) / "exomonad-commands/resources.sock"
