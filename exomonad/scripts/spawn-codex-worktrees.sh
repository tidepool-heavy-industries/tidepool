...
usage() {
  cat <<'EOF'
Usage: exomonad/scripts/spawn-codex-worktrees.sh [options] WORKTREE...

Open one tmux window with one Codex process per worktree, arranged in tiled
panes. A WORKTREE may be a path or a name under .exomonad/worktrees. Every worktree
...
Examples:
  exomonad/scripts/spawn-codex-worktrees.sh \
    actor-test-hygiene repl-name-plane-atomicity

  exomonad/scripts/spawn-codex-worktrees.sh --luna --effort medium \
    actor-test-hygiene

  exomonad/scripts/spawn-codex-worktrees.sh --session overnight --attach \
    .exomonad/worktrees/compiler-endpoint-identity
EOF
}
...
for item in "${requested[@]}"; do
  candidate=$item
  if [[ ! -d $candidate ]]; then
    candidate="$repo_root/.exomonad/worktrees/$item"
  fi
