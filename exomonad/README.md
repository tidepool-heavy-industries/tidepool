# Exomonad field guide

Status: evolving. This is the practical guide for using and improving Exomonad.
...
Exomonad supports ordinary interactive Codex TUIs, frozen workspace customization,
...
all actors; Exomonad owns continuation. The packaged Codex pin is
...
The [workspace package](examples/workspace/README.md) implements the planned
...
`.exomonad` supplies a frozen selection to the whole swarm. `exomonad check --workspace
...
Exomonad is a typed orchestration environment for persistent Codex nodes working
...
`EXOMONAD_INTERACTIVE_CODEX_BIN`. Use Exomonad communication rather than native
...
The default `Tidepool.Actors.Exomonad` facade exposes Codex-node operations rather
...
Exomonad owns the actor tree and reply routing. Hosted Codex processes disable
...
`just exomonad-repl -- --session exomonad-repl-dev` builds this checkout's Exomonad
binary and launches the harness against `~/dev/exomonad-repl` with `gpt-6-astra`
at medium effort. Initialize it with `exomonad new ~/dev/exomonad-repl` first;
the recipe refuses a missing project. This launches
the development harness, not the eventual TUI executable. Use `just exomonad-init`
...
`exomonad/normalization/implementation/domain`, with deterministic numeric
...
- An abnormally terminated root is recreated as a successor actor incarnation
  while the Exomonad host remains alive. The provider conversation survives and
...
- A running Exomonad process does not hot-reload prompts, the shipped Haskell
  library, or runtime code. `just exomonad-console` builds the current checkout,
...
Use the independent, small `~/dev/exomonad-console` repository for the
...
```sh
just exomonad-console -- --session exomonad-console-canary --no-attach
tmux list-panes -t exomonad-console-canary -F '#{pane_id} #{pane_title}'
```
...
research subtree policy in the target project's `.exomonad/config.toml`:
...
socket. See [operator HTTP and graph API](docs/operator-http.md) for provisioning,
