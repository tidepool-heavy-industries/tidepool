...
//! GHC on PATH (`--ignore-default-filter` to run; see `bridge/haskell/CLAUDE.md`'s
...
// (`bridge/mcp/src/eval_prep.rs`); `finalize`'s free result tyvar `a`
...
/// (`exomonad/harness/src/engine.rs`) or `TurnTemplate::render`'s
/// `anchor_result` handling (`bridge/mcp/src/eval_prep.rs`) and this test
