# Synthetic Exomonad judgments

These fixtures test the six applications from the Jev-in-Exomonad example set
...
| --- | --- |
| exomonad-route | `recipient = actor_a`: reviewer of retained offsets |
| exomonad-route-no-match | `recipient = unassigned`: PDF typography is not tax logic |
| exomonad-attention-urgent | Both Nouls favor yes; delay rubric concentrated toward incorrect approval (index 3) |
| exomonad-attention-routine | Both Nouls favor no; delay rubric concentrated toward background information (index 0) |
| exomonad-expand | `step = edge_7`: inspect publication, not cancellation telemetry |
| exomonad-fold-gap | `conclusion = gap`: publication implementation missing |
| exomonad-evidence | `evidence = group_2`: original type mismatch |
| exomonad-evidence-missing | `evidence = no_match`: do not substitute downstream symptoms |
| exomonad-question-duplicate | `relationship = duplicate` |
| exomonad-question-distinct | `relationship = distinct`: HTTP retries versus actor repair |
| exomonad-repair | `next = repair`: reproducible contract violation |
| exomonad-repair-unavailable | `next = owner`: select replacement through coordinator |
| exomonad-experiment | `experiment = trace_invalidation`: differing predictions within budget |
| exomonad-experiment-insufficient | `experiment = none`: remaining plans have identical predictions |
