//! `exomonad-selfharness-web` — the minimal operator GUI server.
//!
//! Serves the operator page ([`exomonad_web::shell`]) and exposes a
//! [`WebGate`](exomonad_web::WebGate) the self-iterating harness driver blocks
...
//! - the default node ([`exomonad_web::DEFAULT_NODE_ID`]) narrates, presents
...
//!
//! Real-driver wiring lives in the sibling `exomonad-selfharness` binary,
//! which boots the same server via
//! [`exomonad_web::spawn_operator_server_multi`] and wires the returned gate
...
/// this binary has no `exomonad-harness` driver instance to borrow it from.
