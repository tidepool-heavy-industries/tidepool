...
`exomonad/harness/tests/companion_collapsed_slice.rs` (seed gate + a forking
turn, scripted record-replay, no live model) and
`exomonad/harness/tests/delegate_positive_path.rs` (the delegating row).
...
```bash
./exomonad/harness-dogfooding/run.sh exomonad/harness-dogfooding/recursive-companion/Harness.hs
```
