...
forwarding <- followWork [("producer", producer, updates)] (\event -> case workMessage id event of { Nothing -> pure Nothing; Just _ -> do { Actor.cast wakes (RoutingCount 1 (const ())); pure Nothing } })
