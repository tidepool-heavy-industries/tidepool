#!/usr/bin/env bash
# Bootstrap the companion's memory store (plans/companion-memory.md): a
# standalone git repo of one-fact-per-file markdown, curated by the PRD 18
# memory agent. Idempotent: refuses to touch an existing store.
#
#   scripts/companion-memory-init.sh [path]     # default: ~/.local/share/tidepool/companion-memory
#
# Then run the harness with TIDEPOOL_MEMORY_REPO=<path>.
set -euo pipefail

STORE="${1:-$HOME/.local/share/tidepool/companion-memory}"

if [ -e "$STORE/.git" ]; then
  echo "already a git repository: $STORE (refusing to reseed)" >&2
  exit 1
fi
...
git init -q
git add -A
git commit -q -m "seed: companion memory store (AGENTS.md curation rules, empty digest)"

echo "seeded: $STORE"
echo "run the harness with: TIDEPOOL_MEMORY_REPO=$STORE"
