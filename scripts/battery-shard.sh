...
#   -E 'binary(companion_collapsed_slice) or binary(answerer_async_fork)'
...
#   -E 'binary(branch_fanout) or binary(compile_fail) or binary(delegate_positive_path) or binary(delegate_type_pinning) or binary(labeled_branch)'
