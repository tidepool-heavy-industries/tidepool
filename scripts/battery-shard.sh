...
#   -E 'binary(companion_collapsed_slice)'
#   -E 'binary(agent_stack_scoping) or binary(answerer_async_fork) or binary(decl_plane_run_scoping) or binary(dogfood_harness_typecheck) or binary(dogfood_observability) or binary(finalize_type_pinning) or binary(fork_child_gui) or binary(golden_path) or binary(outer_effects) or binary(outer_fanout) or binary(outer_subagent) or binary(timing_emission_pin) or binary(turn_lease) or binary(turn_splice) or binary(provider_behavior)'
#   -E 'binary(branch_fanout) or binary(compile_fail) or binary(delegate_positive_path) or binary(delegate_type_pinning) or binary(fork_own_type_pin) or binary(labeled_branch)'
