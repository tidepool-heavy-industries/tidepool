...
#   -E 'binary(selfharness_persistence) or binary(selfharness_fn_finalize_spike)'   # 314s, 16/16 passed (re-verified green at trunk 2026-08-19; an earlier sizing run recorded reds since fixed)
...
#   -E 'binary(agent_stack_scoping) or binary(decl_plane_run_scoping) or binary(dogfood_harness_typecheck) or binary(dogfood_observability) or binary(finalize_type_pinning) or binary(golden_path) or binary(outer_effects) or binary(outer_fanout) or binary(outer_subagent) or binary(timing_emission_pin) or binary(turn_lease) or binary(turn_splice) or binary(provider_behavior)'   # 262s, 56/56 passed (re-verified green at trunk 2026-08-19)
#
...
#   -E 'binary(jit_surface) or binary(resident_session) or binary(nullary_sum_generic_deriving) or binary(patch_crosscheck_differential) or binary(multi_module_datacon) or binary(lsp_graph_walk) or binary(harness_profile_generic_surface) or binary(realm_varid_pinning) or binary(nested_mapm_tag255) or binary(lspnode_show_dot)'   # 239s, 64/64 passed (re-verified green at trunk 2026-08-19)
#   -E 'binary(user_library) or binary(show_double_lens_sigill) or binary(run_llm_turn_sidecar) or binary(session_scope_retirement) or binary(sweep_repoint_smoke) or binary(session_decl_scope_tree) or binary(session_table_qualified_identity) or binary(session_decl_accum)'   # 122s, 87/87 passed (re-verified green at trunk 2026-08-19)
