...
**Known fragility**:
- `Addr#` fallback: a top-level `Addr#` result (for example from `PlusAddr`) returns an empty `LitString` because the bridge cannot recover a length or type from a raw pointer. Double rendering returns managed `Text` and does not use this fallback. [audit-heap-bridge § LitTag::Addr fallback](core-shapes/audit-heap-bridge.md#littagaddr-fallback).
