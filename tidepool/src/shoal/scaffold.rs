//! The Shoal package `shoal new` writes into a project.
//!
//! Every file in it is embedded from one this repository already maintains for
//! its own use — `tidepool/build.rs` generates the table — so a scaffolded
//! workspace and the shipped example cannot drift apart. This module is the
//! only writer of a `.shoal/config.toml`; every other command reads one.
...
/// The configuration `shoal new` writes: today's agent and research defaults,
/// plus the Haskell package — this workspace's own `.shoal` as a source root,
...
# `.shoal/Jev/Operators.hs` in this package fixes its value type to Tidepool's
...
/// Why `shoal new` will not scaffold a path. Each variant leaves the path
/// exactly as it found it.
#[derive(Debug)]
pub enum NewRefusal {
    /// The path already carries a Shoal workspace.
    ...
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::AlreadyAWorkspace(config) => write!(
                formatter,
                "{} already exists, so this is already a Shoal workspace; shoal new will not overwrite one",
                config.display()
            ),
            Self::NotARepositoryRoot(path) => write!(
                formatter,
                "shoal new takes an empty directory or the root of a Git repository, and {} is neither",
                path.display()
            ),
            Self::Occupied(paths) => {
                write!(formatter, "shoal new would overwrite files that already exist, and wrote nothing:")?;
                for path in paths {
                    write!(formatter, "\n  {}", path.display())?;
                }
                Ok(())
...
/// What `shoal new` found at its target path.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(super) enum Target {
    /// Nothing, or an empty directory: `shoal new` creates the repository and
    /// commits the package.
    Fresh,
    /// The root of a Git work tree that carries no Shoal package: the package
    /// is written and staged, and the project owns the commit.
    Repository,
}

/// Produces `flake.lock` for the `flake.nix` `shoal new` writes.
///
...
pub(super) enum JevPin {
    ...
    /// The project brought its own `flake.nix`, which `shoal new` never edits.
    ProjectFlake,
}
...
pub(super) fn scaffold(
    ...
    git.ensure_shoal_local_exclude(workspace)?;
...
/// Decide what `shoal new` is being pointed at, refusing anything it would
/// have to overwrite or guess about.
fn classify(git: &GitCli, workspace: &Path) -> Result<Target, NewRefusal> {
    let config = workspace.join(super::SHOAL_CONFIG);
    ...
    let root = tidepool_worktree::git::inspect::work_tree(git, workspace)
...
fn package_files() -> Vec<(PathBuf, &'static str)> {
    std::iter::once((PathBuf::from(super::SHOAL_CONFIG), CONFIG))
        .chain(
            SCAFFOLD_PACKAGE
                .iter()
...
fn skill_names() -> BTreeSet<&'static str> {
    SCAFFOLD_PACKAGE
        .iter()
        .filter_map(|(path, _)| path.strip_prefix(".shoal/skills/")?.split('/').next())
        .collect()
}
...
/// What a project is missing when the flake `shoal new` wrote could not be
...
/// `shoal new` does not edit.
pub(super) fn project_flake_hint(workspace: &Path) -> String {
    format!(
        "This project has its own flake.nix, which shoal new does not edit. Jev is \
