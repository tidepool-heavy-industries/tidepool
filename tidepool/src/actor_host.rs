                    ...
                    Some(Ok((actor, Ok(thread)))) => {
                        ...
                        if thread.supports_active_input() {
                            match backend.bind_input(&thread).await {
                                Ok(tidepool_agent::InputAdmission::Admitted) => {}
                                Ok(outcome) => {
                                    break Some(format!(
                                        "interactive application {actor:?} native input bind returned {outcome:?}"
                                    ));
                                }
                                Err(error) => {
                                    break Some(format!(
                                        "interactive application {actor:?} could not bind native input control: {error}"
                                    ));
                                }
                            }
                        }
                        if let Err(error) = hosted_retirement::begin_input_seal(
                            &deployment.service,
                            Arc::clone(&backend),
                            thread.clone(),
                        ...
                        {
                            break Some(format!(
                                "interactive application {actor:?} could not retain native input custody: {error}"
                            ));
                        }
...
mod tests {
    ...
    async fn haskell_mailbox_preserves_state_and_opaque_replies_across_calls() {
        ...
        let result = dispatch_haskell_script(
            campaign.root_installation.policy.as_ref(),
            include_str!("actor_host/mailbox_state.hs"),
        )
        .await;
        assert_eq!(result["status"], "committed", "{result:?}");
        for item in result["items"].as_array().unwrap() {
    ...
    async fn haskell_actor_sends_normal_steering_without_a_native_session() {
        ...
        let command = tokio::time::timeout(Duration::from_secs(120), async {
            loop {
                tokio::select! {
                    ...
                    event = campaign.deployments.recv() => match event.unwrap() {
                        LocalResidentDeployment::NotificationSend(command) => break command,
                        LocalResidentDeployment::PolicyInstalled(_) => panic!("Haskell sender acquired a model session"),
                        _ => {}
                    }
                }
            }
        ...
        let finished = dispatch_haskell_script(root.as_ref(), "finished <- awaitExit relay\ncase finished of { Completed (Right _) -> True; _ -> False }").await;
