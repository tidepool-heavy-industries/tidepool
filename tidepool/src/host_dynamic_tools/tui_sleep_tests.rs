    ...
    async fn start(scenario: Scenario) -> Self {
        ...
        let native =
            PathBuf::from(std::env::var_os(NATIVE_ENV).unwrap_or_else(|| {
                panic!("{NATIVE_ENV} must name exact packaged native c76aa485")
            }));
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[ignore = "requires exact packaged native c76aa485, tmux, and an actual 15-minute wait"]
