...
let definition = (stateful "retained-state" ReadOnly (\state (Counter delta answer) -> if delta < 0 then error "handler-probe" else pure (answer state, state + delta)) :: ActorDefinition Int Counter Int)
