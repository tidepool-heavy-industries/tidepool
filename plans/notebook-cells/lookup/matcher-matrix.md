...
- full-sigma matching cannot provide ordinary polymorphic specialization;
- body matching supplies it;
...
symmetric body `tcMatchTy` only when both predicate sets are empty. This is
intentionally conservative: constrained non-exact matches await real predicate
entailment rather than being mislabeled usable.
