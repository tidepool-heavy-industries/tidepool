...
/tmp/tidepool-build-snapshot-codex. The matching native revision is now pinned; see the implementation checkpoint. Preserve
...
acceptance criteria, not a claim that a full live wave has passed.

## 1. Deliver one real original-root to child fork

- [x] Extend the existing workspace custody owner to retain source, build,
...
- [x] Extend AuthorizedForkWorkspace with live-source preparation and an exact
...
- [x] Implement private original-root import under the owned capture boundary,
...
- [x] Assemble/finalize the child's complete view during admission, using checkout
...
- [x] Connect root/prebound launch to the same constructor. Remove the duplicated
...
- [x] Implement managed-source freeze with the shared overlay owner. Preserve
...
- [x] Join source/index capture and the optional fresh build publication under
...
- [x] Keep the existing completed-context fork gate. Source preparation happens
  before it; child Haskell/native execution waits for its normal acknowledgment.
- [x] Exercise a source owned by another actor, an explicit ref, inspection-only
...
- [x] Retain typed transition evidence before starting a mount change. A live
...
- [x] Keep same-sequence replay, exact native identity and cwd refresh. Test lost
...
- [x] Delete restart-only transaction loading, serialized recovery capabilities
...
- [x] Distinguish live namespace routing from durable overlay dependence.
...
- [x] Connect the explicit stable root target seed, or a warm build in the root
...
- [x] Update the shipped shared guide/examples at the new-wave boundary:
...
- [x] Review native writer coverage for the actual selected launch, including
...
- [x] Compile changed consumers, run their focused checks, format and review the
