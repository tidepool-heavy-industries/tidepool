...
| D1-B removal | **done, VERIFICATION INCOMPLETE** (`d1-remove`, folded) | `extract.translate` before/after INCONCLUSIVE (after 93/115/131ms, before 116/122/113ms — no direction, box noise at loadavg 40-46 swamps it). **FINDING: the spec's premise was wrong** — `scanMeta`'s cost was NEVER inside the `translate` bracket; it is a lazy thunk first forced by the UNTIMED `assertMetaCoversEmitted` between the translate and cbor_encode phases. So no instrumentation existed that could have measured this item's win. | `scanMeta` removed from `writeWholeModuleClosed`'s merge (now `[wiredIn, tycon, used, transitive]`); the other two `collectUsedDataCons` call sites (`--all-closed`, per-binding) kept deliberately. `nameById` decoupling LANDED: `collectReachableConDCsRaw` (unfiltered, feeds CHECK A's names) split from `collectReachableConDCs` (filtered, feeds CHECK B only), reason recorded in code at both sites. **WIRE: MOVES IT** — removes a real metadata source; NOT proven inert, since the breadth gates were dropped. | fidelity **30/30** (4 D1Defense checks by name) and acceptance **24/24, 0 skipped** — both COMPLETE but obtained at `42be934d`, a **pre-E6 tree**; quick tier **KILLED mid-compile, zero results → the 3 pinned id-stability tests are UNVERIFIED**; `corpus_report`/differential DROPPED (frozen CBOR, no signal — `COMPARED_FLOOR` never captured); CHECK B: **0 occurrences in every log**; reds: only the sanctioned `qq_fmt`, hit incidentally |
...
| D2 runtime closure | **CUT from the wave** (Inanna, 2026-08-09) — no code, no dev spawned | — | ROUTED FORWARD as a specified, un-started item: `03-d2-handoff.md`. Carries the mandatory-roots hazard with its SUPPLIER CORRECTION (`collectTransitiveDCons`, not `tyconMeta`), the empirical attribution as step one, the derive-from-`freer_names` requirement with its verbatim sentence, the corrected gate set (2 of 4 gates carry no signal), the enumerated pinned trio + the inversion making it D2's real guard, and the `kind=4` ambiguity. | n/a — nothing ran |
