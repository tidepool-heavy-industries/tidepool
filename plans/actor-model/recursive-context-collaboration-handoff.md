...
Status: handoff ready; working-tree implementation is partial and unverified.
...
The preparation changes are currently uncommitted. Before dispatching an
implementation child, commit the reviewed scaffold or explicitly capture the
dirty worktree; seeding from HEAD alone would omit these drafts. Record the
actual seed and final checked candidate rather than treating this brief as
revision evidence.
...
second initializer. Set the intended model/effort in the project's existing
Shoal configuration or pass launch overrides; the initial application campaign
is intended to use Astra. Smaller coding models can complete this preparation.
...
- [ ] Execute role, facade, prompt, mounted-help, and recursive repair checks.
- [ ] Verify launcher preflight and build the matched Shoal binary.
- [ ] Record evidence, review final diff, and commit preparation.
