...
| 14 | `T.takeWhile`/`T.dropWhile` under partial application | `map (T.takeWhile p) ts` | STANDING — silent corruption | Saturated calls correct; PARTIAL application returns inputs unmodified (takeWhile) / empties (dropWhile). FFI intercept only handles saturated calls. `takeWhileT`/`dropWhileT` shadows are LOAD-BEARING; do-not-delegate comments sharpened (82e3f43). |
| 15 | #313 join-wiring (t9/t10 class) | Probe.hs t1-t10 | FIXED | repro313 gate exit 0. |
| 16 | #313 t11 survivor (double `breakOn`) | `Probe.occ2`; **`Patch.patchFile`** | STANDING — root fixing | Second `T.breakOn` on the `sdrop` remainder of a first, in a CROSS-MODULE fn. NEW datapoint: applies to cross-module **M** fns too, not just pure — `patchFile`/`patchJ` trap at runtime ("apply_cont_heap: result con_tag <garbage> is neither Val nor E") on success+ambiguous paths; not-found path fine; same shape INLINE in an eval works. `insertAfter` unaffected. Patch.hs "M-action-inline is fine" comment is stale. Root notified 2026-06-11. |
...
Limits (clean errors)". The two remaining SILENT failures are #14 (takeWhile
PAP — wrong data, no crash) and any not-yet-mapped FFI; everything else is
loud.
...
2. **takeWhile/dropWhile PAP** — silent wrong results; use the Prelude
   shadows (auto via `Tidepool.Prelude`), never `T.takeWhile` partially
   applied.
