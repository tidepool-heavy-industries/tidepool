...
| --- | --- | --- | --- |
| Can display execution be reused across cells or installs? | Display currently compiles a fresh display program per displayed cell. The candidate callable ABI was estimated to remove one roughly 260 ms compiler request, about 100 ms native compilation, and about 187 KB of generated code per displayed cell; remeasure against the final snapshot before relying on those estimates. | Could reduce cumulative generated code and retained programs; may add callable/alias plumbing and mount work. No separate artifact cache or owner is justified yet. | Preserve fresh keys and budgets, single execution, observation-before-display-failure, and partial commits. |
...
The concrete backlog is dominated by repeated compiler work and retained code,
not by an established need for another cache or dispatcher. Request batching has
the strongest measured latency case. Display reuse has a plausible request,
native-compile, and code-size case but needs a fresh matched profile. Actor-spec
reuse and function reclamation need attribution before implementation. Patch
optimization needs a representative production workload before adding a new
runtime boundary. For every candidate, compare runtime latency/throughput,
allocation and retained memory, generated code, artifact/scratch use, build and
test time, dependency fanout, API/control-flow size, failure isolation,
