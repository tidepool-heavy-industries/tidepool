...
///
/// `AskWith` is recognized through the SAME generated `AskReq: FromCore`
/// decode `tidepool-harness`'s `RosterRequest::Ask` uses (§6.2,
/// `crate::generated::ask::AskReq`, generated by `tidepool_protocol` from
/// `tidepool_protocol::effects::ask::ask()`) — not a second, hand-rolled
...
/// failure, fall through only on a name miss" discipline
/// `tidepool-harness::engine::decode_roster`'s `try_member!` macro uses.
///
...
pub fn extract_ask_request(
    ...
    match AskReq::from_value(request, table) {
        Ok(AskReq::AskWith(prompt, payload)) => {
            // Requests arrive fully forced from the JIT bridge, so the aeson
            // Value sub-tree is already materialized — render it directly.
            let meta = Some(value_to_json(&payload, table, 0));
            return Ok((prompt, meta));
        }
        Err(BridgeError::UnknownDataCon(_)) => {} // not an AskWith — try RunLLMTurnWith below
        Err(e) => {
            return Err(format!(
                "ask request malformed (AskWith field decode failed): {e}"
            ));
        }
    }
    ...
    let Value::Con(con_id, fields) = request else {
        return Err(format!(
            "ask received unexpected request shape (expected Con(AskWith|RunLLMTurnWith, ..)): \
             {request:?}"
        ));
    };
    let con_name = table.name_of(*con_id).unwrap_or("<unknown>");
    if con_name != "RunLLMTurnWith" {
        return Err(format!(
