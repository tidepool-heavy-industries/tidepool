...
t5 = do
  Right p <- run "echo hi 1>&2"
  pure (filter (\l -> T.length l > 1) (lines p.stderr))
...
t7 = do
  Right p <- run "echo hi 1>&2"
  pure (filter (\l -> T.length l > 1) (lines p.stderr) <> [pack (show p.exitCode), p.stdout])

t8 :: M [Text]
t8 = do
  _ <- run "true" >>= liftEither
  pure (filter (\l -> T.length l > 1) ["hi", "a"])

t9 :: M Int
t9 = do
  src <- readFile ".tidepool/lib/Tables.hs" >>= liftEither
  let (_, b) = T.breakOn "countTable" src
  pure (if T.null b then 0 else 1)

t10 :: M Int
t10 = do
  src <- readFile ".tidepool/lib/Tables.hs" >>= liftEither
  pure (T.length (T.drop 3 src))
...
occ2 needle hay =
  ...
  in if T.null rest
       then 0
       else let (_, rest2) = T.breakOn needle (T.drop (T.length needle) rest)
            in if T.null rest2 then 1 else 2
