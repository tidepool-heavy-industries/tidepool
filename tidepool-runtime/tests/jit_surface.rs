...
fn works_prelude_core_family() {
    works(
        r#"pure (concat
            ...
            , check "statistics.mean" (mean [1,2,3,4,5::Double] == Just 3.0)
            , check "statistics.mean_empty" (mean ([] :: [Double]) == Nothing)
            , check "statistics.median_odd" (median [1,2,3,4,5::Double] == Just 3.0)
            , check "statistics.median_even" (median [1,2,3,4::Double] == Just 2.5)
            , check "statistics.median_empty" (median ([] :: [Double]) == Nothing)
            , check "statistics.variance" (variance [1,2,3,4,5::Double] == Just 2.5)
            , check "statistics.variance_singleton" (variance [5.0::Double] == Nothing)
            , check "statistics.stdDev" (stdDev [1,2,3,4,5::Double] == Just (sqrt 2.5))
            , check "statistics.percentile_p50" (percentile 50 [1,2,3,4,5::Double] == Just 3.0)
            , check "statistics.percentile_p0" (percentile 0 [1,2,3,4,5::Double] == Just 1.0)
            , check "statistics.percentile_p100" (percentile 100 [1,2,3,4,5::Double] == Just 5.0)
            , check "statistics.percentile_out_of_range" (percentile 101 [1,2,3::Double] == Nothing)
            , check "statistics.percentile_empty" (percentile 50 ([] :: [Double]) == Nothing)
            ])
