...
fn check_node(
    ...
    match (s_node, p_node) {
        ...
        (CoreFrame::App { fun: sf, arg: sa }, CoreFrame::App { fun: pf, arg: pa }) => {
            if sf != pf {
                panic!("node {}: App fun index mismatch ({} vs {})", idx, sf, pf);
            }
            if sa != pa {
                panic!("node {}: App arg index mismatch ({} vs {})", idx, sa, pa);
            }
        ...
        (
            ...
            if s_b != p_b {
                panic!("node {}: Lam body index mismatch ({} vs {})", idx, s_b, p_b);
            }
        ...
        (
            ...
            if s_r != p_r {
                panic!(
                    "node {}: LetNonRec rhs index mismatch ({} vs {})",
                    idx, s_r, p_r
                );
            }
            if s_b != p_b {
                panic!(
                    "node {}: LetNonRec body index mismatch ({} vs {})",
                    idx, s_b, p_b
                );
            }
        ...
        (
            ...
            if s_body != p_body {
                panic!(
                    "node {}: LetRec body index mismatch ({} vs {})",
                    idx, s_body, p_body
                );
            }
            ...
            for ((s_v, s_r), (p_v, p_r)) in s_binds.iter().zip(p_binds.iter()) {
                if s_r != p_r {
                    panic!(
                        "node {}: LetRec binding index mismatch ({} vs {})",
                        idx, s_r, p_r
                    );
                }
        ...
        (
            ...
            if ss != ps {
                panic!(
                    "node {}: Case scrutinee index mismatch ({} vs {})",
                    idx, ss, ps
                );
            }
            ...
            for (j, (s_alt, p_alt)) in s_alts.iter().zip(p_alts.iter()).enumerate() {
                if s_alt.body != p_alt.body {
                    panic!(
                        "node {} alt {}: body index mismatch ({} vs {})",
                        idx, j, s_alt.body, p_alt.body
                    );
                }
                ...
                check_node(
                    s_alt.body, single, s_table, split, p_table, var_map, join_map,
                );
        ...
        (
            ...
            for (&si, &pi) in sf.iter().zip(pf.iter()) {
                if si != pi {
                    panic!("node {}: Con field index mismatch ({} vs {})", idx, si, pi);
                }
        ...
        (
            ...
            if s_r != p_r {
                panic!("node {}: Join rhs index mismatch ({} vs {})", idx, s_r, p_r);
            }
            if s_b != p_b {
                panic!(
                    "node {}: Join body index mismatch ({} vs {})",
                    idx, s_b, p_b
                );
            }
        ...
        (
            ...
            if let Some(&expected_p) = join_map.get(s_l) {
                if expected_p != *p_l {
                    panic!(
                        "node {}: Jump label mismatch. Expected {:?}, got {:?}",
                        idx, expected_p, p_l
                    );
                }
            }
            ...
            for (&sa, &pa) in s_args.iter().zip(p_args.iter()) {
                if sa != pa {
                    panic!("node {}: Jump arg index mismatch ({} vs {})", idx, sa, pa);
                }
                check_node(sa, single, s_table, split, p_table, var_map, join_map);
            }
        }
        (CoreFrame::PrimOp { op: so, args: sa }, CoreFrame::PrimOp { op: po, args: pa }) => {
            ...
            for (&sai, &pai) in sa.iter().zip(pa.iter()) {
                if sai != pai {
                    panic!(
                        "node {}: PrimOp arg index mismatch ({} vs {})",
                        idx, sai, pai
                    );
                }
...
fn compare_alt_con(
    ...
    match (s_con, p_con) {
        ...
        (AltCon::LitAlt(sl), AltCon::LitAlt(pl)) => {
            if sl != pl {
                panic!(
                    "node {} alt {}: literals differ ({:?} vs {:?})",
                    node_idx, alt_idx, sl, pl
                );
            }
        }
        (AltCon::DataAlt(si), AltCon::DataAlt(pi)) => {
            compare_datacon_ids_detailed(
                format!("node {} alt {}", node_idx, alt_idx),
                *si,
                s_table,
                *pi,
                p_table,
        ...
        (s, p) => {
            panic!(
                "node {} alt {}: AltCon variant mismatch (single={:?}, split={:?})",
                node_idx, alt_idx, s, p
            );
        }
    }
}

fn compare_datacon_ids(
    node_idx: usize,
    s_id: DataConId,
    s_table: &DataConTable,
    p_id: DataConId,
...
fn compare_datacon_ids_detailed(
    loc: String,
    s_id: DataConId,
    s_table: &DataConTable,
    p_id: DataConId,
...
pub fn assert_value_equivalent(
    s_val: &Value,
    s_table: &DataConTable,
    p_val: &Value,
    p_table: &DataConTable,
...
fn compare_values(
    ...
    match (s_val, p_val) {
        (Value::Lit(sl), Value::Lit(pl)) => {
            if sl != pl {
                panic!(
                    "value divergence at {}: literals differ ({:?} vs {:?})",
                    format_path(&path),
                    sl,
                    pl
                );
        ...
        (Value::Con(si, sf), Value::Con(pi, pf)) => {
            ...
            if sf.len() != pf.len() {
                panic!(
                    "value divergence at {}: field count mismatch (single={}, split={})",
                    format_path(&path),
                    sf.len(),
                    pf.len()
                );
        ...
        (Value::ByteArray(sb), Value::ByteArray(pb)) => {
            ...
            if *s_bytes != *p_bytes {
                panic!(
                    "value divergence at {}: ByteArray contents differ",
                    format_path(&path)
                );
            }
        }
        (s, p) => {
            panic!(
                "value divergence at {}: variant mismatch (single={}, split={})",
                format_path(&path),
                val_kind(s),
                val_kind(p)
            );
