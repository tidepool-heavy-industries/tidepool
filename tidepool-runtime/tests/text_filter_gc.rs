...
#[test]
fn text_eq_pair() {
    let json = run(r#"(("a" :: T.Text) == ("a" :: T.Text), ("a" :: T.Text) == ("b" :: T.Text))"#);
...
fn text_eq_let_bound() {
    let json = run(r#"let w = "hello" :: T.Text in (w == "hello", w == "world")"#);
    assert_eq!(json, serde_json::json!([true, false]));
}
...
fn int_list_length_twice() {
    let json = run(r#"let xs = [1,2,3] :: [Int] in (length xs, length xs)"#);
    assert_eq!(json, serde_json::json!([3, 3]));
}
...
fn char_list_length_twice() {
    let json = run(r#"let xs = ['a','b','c'] in (length xs, length xs)"#);
    assert_eq!(json, serde_json::json!([3, 3]));
}
...
fn text_singleton_length_twice() {
    let json = run(r#"let xs = ["a"] :: [T.Text] in (length xs, length xs)"#);
...
fn text_separate_lists() {
    let json = run(r#"(length (["a"] :: [T.Text]), length (["b"] :: [T.Text]))"#);
    assert_eq!(json, serde_json::json!([1, 1]));
}
...
fn text_empty_list_twice() {
    let json = run(r#"let xs = [] :: [T.Text] in (length xs, length xs)"#);
...
fn maybe_list_length_twice() {
    let json = run(r#"let xs = [Just (1 :: Int), Nothing, Just 2] in (length xs, length xs)"#);
    assert_eq!(json, serde_json::json!([3, 3]));
}
...
fn pair_list_length_twice() {
    let json = run(r#"let xs = [(1::Int, 2::Int), (3, 4)] in (length xs, length xs)"#);
...
fn text_pack_length_twice() {
    let json = run(r#"let xs = [T.pack "a"] in (length xs, length xs)"#);
    assert_eq!(json, serde_json::json!([1, 1]));
}
...
fn text_singleton_char_length_twice() {
    let json = run(r#"let xs = [T.singleton 'a'] in (length xs, length xs)"#);
    assert_eq!(json, serde_json::json!([1, 1]));
}
...
fn text_list_length_dup_tuple() {
    let json = run(r#"let n = length (["a"] :: [T.Text]) in (n, n)"#);
    assert_eq!(json, serde_json::json!([1, 1]));
}
...
fn int_list_length_dup_tuple() {
    let json = run(r#"let n = length ([1] :: [Int]) in (n, n)"#);
    assert_eq!(json, serde_json::json!([1, 1]));
}
...
fn text_list_length_inline_twice() {
    let json = run(r#"(length (["a"] :: [T.Text]), length (["a"] :: [T.Text]))"#);
    assert_eq!(json, serde_json::json!([1, 1]));
}
...
fn text_list_length_once() {
    let json = run(r#"let xs = ["a", "b"] :: [T.Text] in length xs"#);
    assert_eq!(json, serde_json::json!(2));
}
