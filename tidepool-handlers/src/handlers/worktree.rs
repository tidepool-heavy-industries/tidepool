...
/// block, same seven variants in the same order).
...
/// Build a wire `NotARepository` carrying an explanatory message for a
/// `WorktreeId` that was never registered. See `worktree_lookup`'s doc comment
/// for why this constructor (rather than `WorktreeLost`) is the chosen wire
/// spelling for that case.
fn never_registered(tree_id: &WtWorktreeId) -> WorktreeError {
    WorktreeError::NotARepository(format!(
        "no worktree is registered under id {}",
        tree_id.raw
    ))
}
...
impl WorktreeHandler {
    ...
    /// in this tree.
    ///
    /// A thin delegation to `WorktreeManager::worktree_head` (landed on
    /// root's tip by L1, a fresh `rev-parse HEAD` on every call) — exactly
    /// how `worktree_branch_of` delegates for the branch. That method does
    /// not exist in THIS tree (this worktree forked before it landed and is
    /// deliberately not rebasing), so this is EXPECTED not to compile until
    /// fold integrates the real substrate. Do not stub `worktree_head` under
    /// `tidepool-worktree/` to work around it, and do not reimplement the
    /// `rev-parse` locally — see `worktree_branch_of`'s doc comment and this
    /// method's own doc comment for why a local git call here would be worse
    /// than useless (it would silently reintroduce the exact stale-baseline
    /// gap this verb exists to close).
    #[allow(dead_code)]
    fn worktree_head_of(&mut self, tree_id: WtWorktreeId) -> Result<WtGitOid, WorktreeError> {
        let id = worktree_id_from_wire(&tree_id);
        let head = self.manager.worktree_head(&id).map_err(error_to_wire)?;
