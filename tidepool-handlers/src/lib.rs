...
mod tests {
    ...
    fn kv_clear_prefix_deletes_only_matching_keys() {
        ...
        let deleted_count = match &clear_result {
            Value::Con(id, fields) if table.name_of(*id) == Some("I#") => {
                match &fields[0] {
        ...
        assert_eq!(deleted_count, 2, "kvClear \"ns1/\" should have deleted 2 keys, got {deleted_count}");

        // "ns2/gamma" must still be present.
        let all_keys = response_value(h.dispatch(0, &Value::Con(keys_id, vec![]), &cx).unwrap(), &table);
        ...
        assert_eq!(surviving, vec!["ns2/gamma".to_string()],
            "only ns2/gamma should survive after clearing ns1/; got {:?}", surviving);
    ...
    fn kv_keys_p_filters_by_prefix() {
        ...
        let result = response_value(
            h.dispatch(0, &Value::Con(keysp_id, vec![prefix]), &cx).unwrap(),
            &table,
        );
        ...
        assert_eq!(
            ...
            "kvKeysP \"ns1/\" should return [\"ns1/a\", \"ns1/b\"] sorted; got {:?}", keys
        );
    ...
    fn kv_info_returns_expected_shape() {
        ...
        assert_eq!(name, "Object", "kvInfo should return an Object; got constructor {name:?}");
