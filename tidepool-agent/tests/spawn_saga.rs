...
/// The writable `.git` a spawn grants must come from the RESOLVED worktree's
/// OWN recorded provenance (`WorktreeReceipt.source_repository`), never from
/// the handler-wide `SpawnSubstrate::source_repository` config.
///
/// Two real source repositories, ONE shared registry+binding root (one
/// "handler"): a worktree created against `repo_a` and looked up through a
/// spawner whose manager is configured for `repo_a`, and a SECOND worktree —
/// created against a completely different `repo_b`, but registered in the
/// SAME shared registry — looked up through that SAME spawner via
/// `SpawnWorkspace::Existing`. This is exactly the shape of the production bug:
/// a spawner/handler config naming one repo, resolving a worktree whose real
/// source is another. Each spawn's `CycleSpec.extra_writable_roots` must name
/// its OWN worktree's repo `.git`, never the other one's.
#[test]
fn extra_writable_git_dir_derives_from_the_resolved_worktrees_own_source_repository() {
    let repo_a = TestRepo::init().expect("git init a");
    repo_a
        .writer()
        .commit_file("a.txt", "one", "seed a")
        .expect("seed commit a");
    let repo_b = TestRepo::init().expect("git init b");
    repo_b
        .writer()
        .commit_file("b.txt", "one", "seed b")
        .expect("seed commit b");

    let shared_base = tempfile::TempDir::new().expect("tempdir");
    let registry_root = shared_base.path().join("registry");
    let binding_root = shared_base.path().join("bindings");

    let manager_a = WorktreeManager::new(
        GitCli::new(),
        WorktreeRegistry::open(&registry_root).expect("open registry via a"),
        shared_base.path().join("worktrees"),
        repo_a.path(),
    );
    let manager_b = WorktreeManager::new(
        GitCli::new(),
        WorktreeRegistry::open(&registry_root).expect("open registry via b"),
        shared_base.path().join("worktrees"),
        repo_b.path(),
    );

    // Registered under repo_b's manager, but landing in the SAME shared
    // registry `manager_a` (and the spawner built over it) will look up.
    let worktree_b_id = manager_b
        .create(&WorktreeSpec::from_current_repository("from-b"))
        .expect("create worktree from repo_b")
        .id()
        .clone();

    // ONE handler: a spawner whose manager is configured for repo_a.
    let mut spawner = CoupledSpawner::open(manager_a, &binding_root).expect("open spawner");

    // Spawn 1: a brand-new worktree, created from repo_a through this same
    // spawner. Its recorded source_repository is repo_a.
    let mut backend_a = MockBackend::completing(CycleResultPayload::Absent);
    spawner
        .spawn_one_cycle(
            &mut backend_a,
            &request(
                SpawnWorkspace::New(WorktreeSpec::from_current_repository("worker-a")),
                "worker-a",
            ),
        )
        .expect("spawn against repo_a's own worktree");
    assert_eq!(backend_a.cycles.len(), 1);
    let (_, spec_a) = &backend_a.cycles[0];
    assert_eq!(
        spec_a.extra_writable_roots,
        vec![utf8_path(&repo_a.path().join(".git"))],
        "worker-a's writable .git must be repo_a's, not repo_b's or the handler's default"
    );

    // Spawn 2: the PRE-EXISTING worktree registered from repo_b, resolved
    // through the SAME spawner (whose manager config names repo_a). The
    // handler-wide config must NOT leak into this spawn's writable root.
    let mut backend_b = MockBackend::completing(CycleResultPayload::Absent);
    spawner
        .spawn_one_cycle(
            &mut backend_b,
            &request(SpawnWorkspace::Existing(worktree_b_id), "worker-b"),
        )
        .expect("spawn against the existing repo_b worktree");
    assert_eq!(backend_b.cycles.len(), 1);
    let (_, spec_b) = &backend_b.cycles[0];
    assert_eq!(
        spec_b.extra_writable_roots,
        vec![utf8_path(&repo_b.path().join(".git"))],
        "worker-b's writable .git must be repo_b's OWN recorded source, not repo_a's handler config"
    );

    assert_ne!(
        spec_a.extra_writable_roots, spec_b.extra_writable_roots,
        "two spawns against two different source repositories must never share a writable .git"
    );
}

fn utf8_path(path: &Path) -> String {
    camino::Utf8Path::from_path(path)
        .expect("test repo paths are UTF-8")
        .as_str()
        .to_string()
}
