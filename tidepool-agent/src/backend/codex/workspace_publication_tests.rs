...
async fn exact_publication_request_and_lost_reply_are_not_retried() {
    for respond in [true, false] {
        ...
        let server = tokio::spawn(async move {
            ...
            assert_eq!(
                payload,
                serde_json::json!({"threadId":"bound-thread", "sequence":7, "operation":"begin"})
            );
            if respond {
                let body = r#"{"status":"ready","pid":123,"cgroupPath":"/sys/fs/cgroup/writers"}"#;
        ...
        if respond {
            assert!(matches!(
                reply,
                Ok(PublicationReply::Ready { pid: 123, .. })
            ));
        } else {
            assert!(matches!(
                reply,
