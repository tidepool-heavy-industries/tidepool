...
fn test_gc_fires_and_program_computes() {
    std::thread::Builder::new()
        ...
        .spawn(|| {
            // Shallow case: 2 KiB nursery, depth-40 Con chain should require
            // GC but still succeed.
            {
            ...
            // Deep case: same nursery, depth-60 chain — forces multiple GC
            // cycles and verifies the resulting nested Con(...Lit(42)) spine
            // survives them intact.
            {
                ...
                } else if let Err(JitError::Yield(YieldError::Runtime(
