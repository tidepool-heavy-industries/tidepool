...
mod group1_header {
        ...
        for &tag in &tags {
            for &size in &sizes {
                ...
                unsafe {
                    ...
                    assert_eq!(read_size(guard.ptr), size as u16, "Size mismatch for size {}", size);
                }
            }
        }
    }
    ...
    proptest! {
        #![proptest_config(ProptestConfig { cases: 300..=500, ..ProptestConfig::default() })]
        #[test]
    ...
    fn test_padding_asymmetry() {
        ...
        for &size in &sizes_to_test {
            ...
            unsafe {
                ...
                if size >= 8 {
                    assert_eq!(padding, &[0, 0, 0, 0, 0], "Padding should be zeroed for size {}", size);
                } else {
                    assert_eq!(padding, &[0xAA, 0xAA, 0xAA, 0xAA, 0xAA], "Padding should NOT be zeroed for size {}", size);
                }
                dealloc(ptr, layout);
            }
        }
...
mod group3_lit {
            ...
            unsafe {
                ...
                *guard.ptr.add(LIT_TAG_OFFSET) = tag_val;
                ...
                if tag_val <= 4 {
                    ...
                    assert!(heap_tag.is_none(), "Tag {} should be None in heap-side LitTag", tag_val);
                }
            }
        }
    }
    ...
    fn test_lit_shared_constants() {
        ...
        assert_eq!(CON_NUM_FIELDS_OFFSET, cg_layout::CON_NUM_FIELDS_OFFSET as usize);
        assert_eq!(CON_FIELDS_OFFSET, cg_layout::CON_FIELDS_OFFSET as usize);
        assert_eq!(CLOSURE_CODE_PTR_OFFSET, cg_layout::CLOSURE_CODE_PTR_OFFSET as usize);
        assert_eq!(CLOSURE_NUM_CAPTURED_OFFSET, cg_layout::CLOSURE_NUM_CAPTURED_OFFSET as usize);
        assert_eq!(CLOSURE_CAPTURED_OFFSET, cg_layout::CLOSURE_CAPTURED_OFFSET as usize);
        assert_eq!(THUNK_STATE_OFFSET, cg_layout::THUNK_STATE_OFFSET as usize);
        assert_eq!(THUNK_CODE_PTR_OFFSET, cg_layout::THUNK_CODE_PTR_OFFSET as usize);
        assert_eq!(THUNK_CAPTURED_OFFSET, cg_layout::THUNK_CAPTURED_OFFSET as usize);
        assert_eq!(THUNK_INDIRECTION_OFFSET, cg_layout::THUNK_INDIRECTION_OFFSET as usize);
...
mod group4_bytearray {
    ...
    fn test_byte_array_resize_content_preservation() {
        ...
        unsafe {
            ...
            // Grow
            let new_n = 200;
            let new_ba = runtime_resize_byte_array(ba, new_n as i64);
            ...
            // Shrink via resize
            let newer_n = 50;
            let newer_ba = runtime_resize_byte_array(new_ba, newer_n as i64);
        ...
        let verdict = fork_contained(|| {
            ...
            unsafe {
                ...
                *(thunk_ptr.add(THUNK_CAPTURED_OFFSET) as *mut *mut u8) = lit_ptr;
                ...
                let mut to_space = std::slice::from_raw_parts_mut(to_guard.ptr, 1024);
                cheney_copy(&roots, from_guard.ptr, from_guard.ptr.add(1024), &mut to_space);
                ...
                assert_eq!(captured_in_new, lit_ptr, "Lit should NOT have been evacuated");
            }
        });
        assert_eq!(verdict, ForkVerdict::Pass);
    }
    ...
    fn bug_c2_con_writer_wrap_gc() {
        let verdict = fork_contained(|| {
            ...
            unsafe {
                ...
                let roots = [&mut root as *mut *mut u8];
                let mut to_space = std::slice::from_raw_parts_mut(to_guard.ptr, 70000);
                let res = cheney_copy(&roots, from_guard.ptr, from_guard.ptr.add(70000), &mut to_space);
                
                assert_eq!(res.bytes_copied, 0, "Evacuate copied {} bytes but object has 8189 fields", res.bytes_copied);
            }
        });
        assert_eq!(verdict, ForkVerdict::Pass);
    }
