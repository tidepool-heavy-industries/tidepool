...
//! immutable tool surface and submits invocations to the owning local actor.
...
impl ResidentMcpClient {
    ...
    pub(crate) async fn dispatch(
        ...
        self.actor
            ...
            .map_err(|_| ResidentMcpError::Unavailable("the owning actor has stopped".into()))?;
        receive
            .await
            .map_err(|_| {
    ...
    pub(crate) async fn dispatch_workbench(
        ...
        let response = response.map_err(|error| ResidentMcpError::Failed(error.to_string()))?;
        serde_json::to_value(response).map_err(|error| ResidentMcpError::Failed(error.to_string()))
    }
}
