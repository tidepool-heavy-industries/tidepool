...
pub fn eval_cbor(path: &str) -> Result<Value, Box<dyn Error>> {
    ...
    let expr = tidepool_repr::serial::read::read_cbor(&bytes)?;
    let mut heap = VecHeap::new();
...
mod tests {
    use super::*;
    use tidepool_repr::{Alt, AltCon, DataConId, JoinId, Literal, PrimOpKind, VarId};
    ...
    fn test_lam_compose() {
        ...
        let nodes = vec![
            ...
            CoreFrame::Lam {
                binder: VarId(2),
                body: 5,
            }, // 6: lambda g. lambda x. f (g x)
            CoreFrame::Lam {
                binder: VarId(1),
                body: 6,
            }, // 7: lambda f. lambda g. lambda x. f (g x)
            // f = lambda n. n + 1
