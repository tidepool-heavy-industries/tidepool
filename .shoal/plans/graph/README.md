...
or feature, replace the application allocation while reusing that working style.
...
For this example, a **relation view** chooses which observed relationship supplies
display edges; it grants no authority and leaves raw facts intact. A **presentation
root** is a root in that display forest. A missing parent or cycle break does not
turn an actor into a runtime root. These are application terms, separate from the
generic workbench vocabulary.

An initial Astra planner collaborates with the human on the product intent and
reviews the Sol owners' interpretations of this plan and its Haskell vocabulary.
A Sol integration owner pilots it, and Sol component leads own delivery. One tagged Astra specialist
...
Before broad implementation, the selected Sol owners commit their own execution
plans and publish cumulative questions. The original planner reviews coupled
choices and returns explicit release scope. Preserve the complete product
obligation across this checkpoint and subsequent local waves.

1. Sol contract lead owns [the shared contract](contract/README.md).
   It first consults the [tagged Astra design leaf](contract/design.md),
...
2. The Sol integration owner incorporates that exact result into its integration
   ...
   [projection](projection/README.md) and [controls](controls/README.md)
...
3. Each lead owns its implementation, useful recursive subtrees and successive
   scaffold/integration waves. Its reviewer returns findings for
...
4. Feature acceptance requires all three relationship views in the real client,
