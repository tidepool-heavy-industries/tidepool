...
A root in the human's planning conversation runs on the shipped `root.md`; there
is no separate planner prompt to select, and the local copy that used to supply
one was removed because it had drifted into telling the root to hand off and go
idle. Follow the actor's actual input/result and authority.
