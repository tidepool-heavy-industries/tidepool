...
before <- snapshot
leadWork <- unfold (taskGroup task) (childWithProgress @Attention @Delivery (withLifetime SwarmOwned (withContext (selected taskContext) (componentLeadFrom leadLabel projectHead task))))
