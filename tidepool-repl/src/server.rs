    ...
    pub async fn dispatch_tool(
        ...
        match name {
            ...
            "session_def" => {
                ...
                let sid = resolve_session(req.session);
                Ok(self
                    .run_command("session_def", SessionCommand::Def(DeclText(req.decl)), &sid)
                    .await)
            }
            "session_eval" => {
                ...
                let sid = resolve_session(req.session);
                Ok(self
                    .run_command("session_eval", SessionCommand::Eval(ExprText(req.code)), &sid)
                    .await)
            }
            "session_cmd" => {
                ...
                Ok(self
                    .run_command("session_cmd", SessionCommand::Cmd(meta), &sid)
                    .await)
            }
            "session_close" => {
