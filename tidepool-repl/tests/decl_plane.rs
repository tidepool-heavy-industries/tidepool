...
/// PURE-BIND collision must SHADOW gracefully (ledger #36), not raise an
/// "Ambiguous occurrence". A pure `let` whose name collides with an in-scope
/// import (`reverse` from the Prelude — used recursively) now binds via the
/// SAME `hiding (…)`-guarded decl route a genuine top-level decl uses (GHCi
/// parity), so pure and effectful binds stay interchangeable: the session name
...
async fn colliding_pure_bind_shadows_gracefully() {
    ...
    // must SHADOW it (GHCi parity), not raise an ambiguous occurrence.
    repl.eval("let reverse Leaf = []; reverse (Node l x r) = reverse r ++ [x] ++ reverse l")
        .await
        .expect_ok("shadowing pure bind must succeed");
    let v = repl
        .eval_ok("pure (reverse (Node Leaf (7 :: Int) Leaf))")
        .await;
    assert!(v.contains('7'), "shadowed reverse usable after bind: {v}");
    ...
    let w = repl
        ...
        .await;
    assert!(w.contains('7'), "treeList usable after: {w}");
}
