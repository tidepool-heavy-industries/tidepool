...
fn guide_md(ctx: &ResourceCtx) -> String {
    let mut s = String::from(concat!(
        ...
        "\n## Querying JSON files\n",
        "The canonical pattern is `readGlob` + `eitherDecode` + optics (`key`/`_String`/`values`/`cosmos`), ",
        "not `grepGlob`: decode each file to a `Value` and walk it with lenses.\n",
        "```haskell\n",
        "do { rs <- readGlob \"data/*.json\"\n",
        "   ; pure [ v ^? key \"status\" . _String\n",
        "          | r <- rs, Right t <- [r.contents], Right v <- [eitherDecode t] ] }\n",
        "```\n",
        "`grepGlob`'s `Hit` is one matched LINE — a good fit for source code, a poor one for a ",
        "single-line/minified JSON file, where the whole file is one \"line\": that hit's `text` is now ",
        "truncated with a marker pointing back here rather than returned in full.\n",
        "\n## Pagination\n",
...
mod tests {
    ...
    fn help_resolves_topics() {
        ...
        // `llmRaw` verb wrapper is decl-generated and shows up in the
        // per-effect resource.
