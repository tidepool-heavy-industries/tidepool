    ...
    ($project:path) => {
        $project! {
            ...
            helpers [
                ...
                // `returnControl`/`returnControlFork`/`returnControlFanout` are the
                ...
                // `returnControlSited`/`returnControlForkSited`/
                // `returnControlFanoutSited` are reachable (referencing them here is
                ...
                // returnControlFanout (B1, widen): one park, N thunk children — the
                // SAME classification scheme as returnControlFork ("typedSite" +
                ...
                { raw ["{-# OPAQUE returnControlFanout #-}",
                       "returnControlFanout :: forall a. [Text] -> M [a]",
                       "returnControlFanout prompts = returnControlFanoutSited 0 prompts"] },
                ...
                { raw ["{-# OPAQUE returnControlFanoutSited #-}",
                       "returnControlFanoutSited :: forall a. Int -> [Text] -> M [a]",
                       "returnControlFanoutSited sid prompts = unsafeCoerce <$> send (AskWith (intercalate \"\\n\" prompts) (object [\"typedSite\" .= sid, \"fork\" .= True, \"fan\" .= length prompts, \"prompts\" .= prompts]))"] },
