    ...
    ($project:path) => {
        $project! {
            ...
            prompt_card [
                ...
                "constructors are direct options; a payload constructor like `Other Text` is a ",
                "selectable branch with its fields), or a primitive (`Text`/`Int`/`Bool`); a bad ",
                "submission re-prompts internally, no `Either` to unwrap: `d <- askUser @Deploy`, ",
                "`k <- askUser @NextStep` then `case k of ...` to sequence follow-ups.\n",
