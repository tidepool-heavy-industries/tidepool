...
// ---------------------------------------------------------------------------
// GHC error location rewriting (session items + REPL paths)
// ---------------------------------------------------------------------------

/// Compute the user-content line offset for a generated eval-module source.
///
/// Returns the number of lines BEFORE the first user-content line (so user
/// content starts at generated line `offset + 1`).  Three anchors are tried in
/// order — whichever matches first determines the offset:
///
/// 1. `__user =\n` — the `template_haskell` / `wrap_pure_ref_source` anchor.
/// 2. `result = do\n` — the `wrap_bind_source` / `wrap_multi_bind_source` anchor.
/// 3. `-- [user]\n` — the common fallback (the marker emitted by
///    `begin_user_module`; user content starts on the NEXT line).
pub fn eval_user_offset(source: &str) -> usize {
    if let Some(pos) = source.find("__user =\n") {
        return source[..pos + "__user =\n".len()].matches('\n').count();
    }
    if let Some(pos) = source.find("result = do\n") {
        return source[..pos + "result = do\n".len()].matches('\n').count();
    }
    source
        .find("-- [user]\n")
...
/// Compute the user-content line offset for a decl-module source produced by
/// `render_module`.
///
/// The module has a header (pragmas → module line → export list → `) where`)
/// followed by imports, a blank line, then user declarations.  The offset is
/// the number of generated lines before the first user-declaration line.
pub fn decl_user_offset(module_source: &str) -> usize {
    // `) where\n` ends the module header; imports follow; `\n\n` marks the
    // blank separator between imports and user content.
    let where_end = module_source
        .find(") where\n")
        .map(|p| p + ") where\n".len())
        .unwrap_or(0);
    let imports_area = &module_source[where_end..];
    if let Some(blank) = imports_area.find("\n\n") {
        // `+2` to count both newlines of the blank line separator.
        let abs = where_end + blank + 2;
        module_source[..abs].matches('\n').count()
    } else {
        module_source[..where_end].matches('\n').count()
    }
}

/// Extract the `.hs` file basename (e.g. `"G1.hs"`) from a rendered decl-module
/// source.  Decl modules have a header `module Tidepool.Session.Lib.G<n> (`; the
/// last dotted component with `.hs` appended is the basename GHC uses in its
/// diagnostics.
pub fn decl_module_basename(module_source: &str) -> String {
    module_source
        .lines()
        ...
        .map(|base| format!("{base}.hs"))
        .unwrap_or_else(|| "G1.hs".to_string())
}

/// Rewrite GHC error location spans in `error_text` to item-relative coordinates.
///
/// Every occurrence of `[<path>/]<name>.hs:<L>:<C>` (or `…:<L>:<C-D>` ranges) is
/// processed:
///
/// - If `<name>.hs` == `user_file` **and** `L > user_offset`: strip the path,
///   emit `L-user_offset:C` (item-relative line, no filename).
/// - Otherwise: strip the path, emit `[wrapper] <name>.hs:L:C`.
///
/// A second pass rewrites caret-block source lines (`  N | source`) whose N
/// appears in the remapped line set.
///
/// `user_offset` is the number of generated-module lines that precede the first
/// user-content line (i.e. from [`eval_user_offset`] or [`decl_user_offset`]).
pub fn rewrite_ghc_errors(error_text: &str, user_file: &str, user_offset: usize) -> String {
    let mut line_map = std::collections::HashMap::new();
    let pass1 = rewrite_location_spans(error_text, user_file, user_offset, &mut line_map);
    if line_map.is_empty() {
        return pass1;
    }
    rewrite_caret_lines(&pass1, &line_map)
}

fn rewrite_location_spans(
    text: &str,
    user_file: &str,
    user_offset: usize,
    line_map: &mut std::collections::HashMap<usize, usize>,
) -> String {
    const HS_COLON: &str = ".hs:";
    let mut out = String::with_capacity(text.len() + 64);
    let mut remaining = text;

    loop {
        let Some(found) = remaining.find(HS_COLON) else {
            out.push_str(remaining);
            break;
        };

        let after_colon_pos = found + HS_COLON.len();

        // Require a digit immediately after ".hs:" — otherwise not a location.
        if !remaining[after_colon_pos..]
            .starts_with(|c: char| c.is_ascii_digit())
        {
            out.push_str(&remaining[..after_colon_pos]);
            remaining = &remaining[after_colon_pos..];
            continue;
        }

        // Walk back through everything before ".hs" to find where the file
        // path starts (immediately after whitespace / paren / quote / comma).
        let before_hs = &remaining[..found];
        let path_local_start = before_hs
            .rfind(|c: char| c.is_ascii_whitespace() || matches!(c, '(' | '"' | '\'' | ','))
            .map(|i| i + 1)
            .unwrap_or(0);

        let full_path_no_ext = &before_hs[path_local_start..];
        let basename_no_ext = full_path_no_ext
            .rfind('/')
            .map(|i| &full_path_no_ext[i + 1..])
            .unwrap_or(full_path_no_ext);
        let basename = format!("{basename_no_ext}.hs");

        // Parse line number from after ".hs:".
        let after_hs_colon = &remaining[after_colon_pos..];
        let line_end = after_hs_colon
            .find(|c: char| !c.is_ascii_digit())
            .unwrap_or(after_hs_colon.len());
        let line: usize = after_hs_colon[..line_end].parse().unwrap_or(0);

        // Parse optional column — ":col" or ":col-col".
        let after_line = &after_hs_colon[line_end..];
        let (col_text, col_consumed) = if let Some(col_digits) = after_line.strip_prefix(':') {
            let digit_end = col_digits
                .find(|c: char| !c.is_ascii_digit())
                .unwrap_or(col_digits.len());
            if digit_end > 0 {
                // Optional "-N" range suffix.
                let range_len = {
                    let tail = &col_digits[digit_end..];
                    if let Some(rd) = tail.strip_prefix('-') {
                        let rend = rd
                            .find(|c: char| !c.is_ascii_digit())
                            .unwrap_or(rd.len());
                        if rend > 0 { 1 + rend } else { 0 }
                    } else {
                        0
                    }
                };
                let total = 1 + digit_end + range_len; // ":" + digits + range
                (&after_line[..total], total)
            } else {
                ("", 0)
            }
        } else {
        ...
        // Emit the text that preceded the path (everything before it).
        out.push_str(&remaining[..path_local_start]);

        // Emit the remapped location (or wrapper label).
        if basename == user_file && line > user_offset {
            let item_line = line - user_offset;
            line_map.insert(line, item_line);
            out.push_str(&item_line.to_string());
            out.push_str(col_text);
        } else if !basename_no_ext.is_empty() {
            ...
            // Degenerate: empty basename — emit original span unchanged.
            out.push_str(&remaining[path_local_start..after_colon_pos + line_end + col_consumed]);
        }

        remaining = &after_hs_colon[line_end + col_consumed..];
    }

    out
}

fn rewrite_caret_lines(
    text: &str,
    ...
    loop {
        match remaining.find('\n') {
            None => {
                out.push_str(&rewrite_one_caret_line(remaining, line_map));
                break;
            }
            Some(nl) => {
                out.push_str(&rewrite_one_caret_line(&remaining[..nl], line_map));
                out.push('\n');
...
fn rewrite_one_caret_line(
    line: &str,
    line_map: &std::collections::HashMap<usize, usize>,
) -> String {
    // GHC caret-block source lines: `  40 | source code`
    // Pattern: leading spaces, then digits, then " | ".
    let trimmed = line.trim_start();
    let digit_end = trimmed
        .find(|c: char| !c.is_ascii_digit())
        .unwrap_or(0);
    if digit_end > 0 && trimmed[digit_end..].starts_with(" | ") {
        if let Ok(orig) = trimmed[..digit_end].parse::<usize>() {
            if let Some(&new_n) = line_map.get(&orig) {
                let leading = line.len() - trimmed.len();
                let new_str = new_n.to_string();
                // Right-align new number in field of width `digit_end`.
                let pad = digit_end.saturating_sub(new_str.len());
                return format!(
                    "{}{}{}{}",
                    " ".repeat(leading),
                    " ".repeat(pad),
...
mod tests {
    ...
    // ---- rewrite_ghc_errors unit tests ----

    /// Real-shape GHC error fixture: a type-mismatch on line 40 of the generated
    /// module where user content starts at offset 39 (line 40 = item line 1).
    const FIXTURE_GHC_ERROR: &str = concat!(
        "/tmp/.tmpQLg3yc/Expr.hs:40:5: error: [GHC-83865]\n",
        "    \u{2022} Couldn't match expected type ‘Int’ with actual type ‘Text’\n",
        "   |\n",
        "40 | badExpr\n",
        "   | ^^^^^^^\n",
        "/tmp/.tmpQLg3yc/Expr.hs:40:15: error: [GHC-12345]\n",
        "    \u{2022} Variable not in scope: foo\n",
        "   |\n",
        "40 | badExpr\n",
        "   |               ^^^\n",
    );
    ...
    #[test]
    fn rewrite_strips_tmp_path_and_remaps_line() {
        // user_offset=39: line 40 → item line 1
        let out = rewrite_ghc_errors(FIXTURE_GHC_ERROR, "Expr.hs", 39);
        assert!(
            !out.contains("/tmp/"),
            "tmp path should be stripped: {out}"
        );
        assert!(
            !out.contains("Expr.hs:"),
            "filename should be stripped for user lines: {out}"
        );
        assert!(
            out.contains("1:5:"),
            "line 40 with offset 39 should become 1:5: {out}"
        );
        assert!(
            out.contains("1:15:"),
            "second error line 40 col 15 should become 1:15: {out}"
        );
    }

    #[test]
    fn rewrite_remaps_caret_line_numbers() {
        let out = rewrite_ghc_errors(FIXTURE_GHC_ERROR, "Expr.hs", 39);
        // The caret block " 40 | badExpr" should have 40 remapped to 1.
        assert!(
            out.contains(" 1 | badExpr"),
            "caret block line 40 should be remapped to 1: {out}"
        );
        // The blank pipe line should be unchanged.
        assert!(out.contains("   |"), "blank pipe line should remain: {out}");
    }

    #[test]
    fn rewrite_labels_wrapper_errors() {
        // Line 5 with offset 39 → 5 < 39, so labeled [wrapper].
        let input =
            "/tmp/.tmpXXX/Expr.hs:5:1: error: [GHC-00000]\n    • import error\n";
        let out = rewrite_ghc_errors(input, "Expr.hs", 39);
        assert!(
            out.contains("[wrapper] Expr.hs:5:1"),
            "preamble-origin errors should be labeled [wrapper]: {out}"
        );
        assert!(
            !out.contains("/tmp/"),
            "tmp path should still be stripped: {out}"
        );
    }

    #[test]
    fn rewrite_passes_through_no_hs_locations() {
        let input = "Some error with no file references\n";
        let out = rewrite_ghc_errors(input, "Expr.hs", 10);
        assert_eq!(out, input);
    }

    #[test]
    fn rewrite_bare_filename_without_path() {
        // Bare "Expr.hs:3:5" (no /tmp/ prefix) — strip filename, remap.
        let input = "Expr.hs:3:5: error: oops\n";
        let out = rewrite_ghc_errors(input, "Expr.hs", 2);
        assert_eq!(out, "1:5: error: oops\n", "got: {out}");
    }

    #[test]
    fn rewrite_different_user_file() {
        // A G1.hs error: line 11, offset 10 → item line 1.
        let input = "/path/to/Tidepool/Session/Lib/G1.hs:11:3: error: type mismatch\n";
        let out = rewrite_ghc_errors(input, "G1.hs", 10);
        assert_eq!(out, "1:3: error: type mismatch\n", "got: {out}");
    }

    #[test]
    fn eval_user_offset_template_haskell_path() {
        let source = "module Expr where\nimport X\n-- [user]\n__user =\n  pure 42\n";
        // "__user =\n" is on line 4 (1-based); offset = 4 (lines before that + "__user =\n" itself).
        assert_eq!(eval_user_offset(source), 4);
    }

    #[test]
    fn eval_user_offset_bind_path() {
        let source = "module Expr where\nimport X\n-- [user]\nresult :: Eff _ _\nresult = do\n  x <- e\n";
        // "result = do\n" is line 5; offset = 5.
        assert_eq!(eval_user_offset(source), 5);
    }

    #[test]
    fn decl_user_offset_basic() {
        let source = concat!(
            "{-# LANGUAGE Foo #-}\n",        // line 1
            "-- GENERATED A\n",               // line 2
            "-- GENERATED B\n",               // line 3
            "module Tidepool.Session.Lib.G1 (\n", // line 4
            "    slug\n",                     // line 5
            "  ) where\n",                    // line 6
            "import Prelude\n",               // line 7
            "\n",                             // line 8 blank
        ...
        // User content starts at line 9; offset should be 8.
        assert_eq!(decl_user_offset(source), 8);
    }

    #[test]
    fn decl_module_basename_extracts_g1() {
        let source = "module Tidepool.Session.Lib.G1 (\n    slug\n  ) where\n";
        assert_eq!(decl_module_basename(source), "G1.hs");
    }

    #[test]
    fn decl_module_basename_fallback() {
        assert_eq!(decl_module_basename("no module here"), "G1.hs");
    }
