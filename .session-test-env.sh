...
#   ( cd "$(git rev-parse --show-toplevel)/bridge/haskell" && cabal build tidepool-extract-bin )
# ). Run tests via:
#   nix develop --command bash -lc '. .session-test-env.sh && cargo test ...'
set -a
REPO="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
TIDEPOOL_EXTRACT="$REPO/target/debug/tidepool-extract"
TIDEPOOL_EXTRACT_WORKER="$(cd "$REPO/bridge/haskell" 2>/dev/null && cabal list-bin tidepool-extract-bin 2>/dev/null)"
...
echo "worker:   ${TIDEPOOL_EXTRACT_WORKER:-MISSING (run: cd bridge/haskell && cabal build tidepool-extract-bin)}"
