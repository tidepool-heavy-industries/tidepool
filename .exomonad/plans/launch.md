...
Use a fixed checked Exomonad executable. In the application checkout, first run
`exomonad check --workspace .` to compile the authored selection without models.
Run `exomonad check --workspace . --recipes` to execute the configured package checks
...
```sh
exomonad init --workspace /path/to/project --session new-authorized-session
```
...
root's .exomonad is authoritative. Candidate files in managed checkouts activate only
...
Project.RoutingChecks; their GHCi expressions are adjacent in .exomonad/checks.
Edit a helper, its guidance and its checks together, then run:

```sh
exomonad check --workspace . --recipes
```
...
seeded with authored .exomonad files. They create their own source fixtures; they do
