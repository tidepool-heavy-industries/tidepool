...
| 30 | SHOW correctness: a negative `Double` in constructor-argument position is NOT parenthesized — `show (Just (-2.5))` → `Just -2.5` (GHC: `Just (-2.5)`). Negative `Int` is CORRECT (`Just (-1)`). Breaks `read . show` round-trips; affects any structure with negative Double fields (records/tuples/Maybe/lists). ROOT: `emitShowDoubleSpecBody` (Translate.hs) replaces `$fShowDouble_$sshowSignedFloat` with `\showPos p d rest -> ShowDoubleAddr d ++ rest`, DROPPING the real `showSignedFloat`'s `showParen (p > 6)` when `x < 0`. ATTEMPTED 2026-07-02 (reverted): built a `ShowSignedDoubleAddr(prec,d)` primop that parenthesizes in the RUST host (`runtime_show_signed_double_addr` — no hand-emitted Core compare, so the SIGILL risk is gone; primop+eval+host plumbing all worked). BUT the fix STALLED on precedence availability: the hot path is the CALL-SITE interception in `translate` (Translate.hs ~L895, not the binding-level `emitShowDoubleSpecBody`), and there the 2nd arg is a Var reference to `minExpt [GHC.Internal.Float]` which `Resolve.hs:311` DELIBERATELY BLOCKS (it culls showSignedFloat + its deps to avoid floatToDigits) → translating it = "Dangling NVar: minExpt". So `args !! 1` is NOT a clean precedence literal here; the precedence is entangled with the blocked machinery (possibly the specialized `$s` arg order differs from vanilla `showSignedFloat (fmt,p,d,rest)`). NEXT: dump the closed Core of `$fShowDouble_$sshowSignedFloat` (needs the real -O2 corpus / differential harness) to find WHERE the precedence actually is — either a different arg, or reconstruct it. The Rust-host primop approach is the right shape once the precedence is located; keep it. | 2026-07-02 |
