...
| 21 | Decl responses under-paint: `{decl:"heatOf"}` omits the inferred type the server had at compile time — cost a `:t` round-trip. Notebook-frame criterion: render every mutation fully, once, at mutation time. Also: an item whose decl leads with imports reports `{decl:"import"}` (head-name picker takes the first token); an OPERATOR def (`x <+> y = …`) reports `{decl:""}` (empty), and a fixity decl (`infixl 6 <+>`) reports `{decl:"infixl"}` (the keyword) — all the same first-token head-picker bug (2026-07-02 kata sweep) | audit the response shapes against render-once (with `stale:` field + `:program` repaint, per the coupled-context design conversation) | 2026-07-01 |
...
## Beta-user harvest (2026-07-03 — 3 Fable sessions: 2× repl, 1× single-shot eval)

Three independent "external beta user" reports. Ranked by CONVERGENCE (how many
of the 3 hit it) × severity. Convergent = highest signal.

**Convergent (all 3 users):**
1. **`input` lane BROKEN for `let`/pure-binds — REGRESSION from this session's M2 pure-bind→decl lowering.** `let x = input…` / `x <- pure (input…)` → "Variable not in scope: input" because the RHS was lowered to the DECL plane, which doesn't carry `input`. The tool description explicitly promises input works in lets/binds. Grim workaround users found: `payload <- fmap (const input) (pure ())`. FIX: don't lower a bind whose RHS mentions `input` (keep it materialized/stmt-plane) — in `try_pure_bind_as_decl`/routing (session.rs). Also fix the error msg + the doc. 🔴 highest priority; I own it.
2. **Lib-module poisoning bricks the WHOLE plane** (ledger #24, hit live by all 3): a concurrent agent's broken `.tidepool/lib/*.hs` killed every eval because `Library` auto-imports it. AND `defMod`'s scaffold is STALE (emits `NoImplicitPrelude, OverloadedStrings` + `import Tidepool.Prelude`; canonical needs `OverloadedRecordDot`, `hiding (error)`, `import Tidepool.Effects`) → the happy path MANUFACTURES brick-risk (no `M`, no `L.`, `m.size` parses as composition). FIX: compile-gate-before-register for lib writes; fix defMod scaffold; fire the curated "your item is fine — a lib module is broken" message on the FIRST failure (today only on retry, on both planes).
3. **Effect-verb discoverability = name-guessing roulette:** `kvPut→kvSet`, `now→getCurrentTime`, `ask` arity (`Schema->Text->M Value`), `parseJson` is monadic (reads pure). Best verbs (update family, rule-DSL `rPat/?>/<?`, Schema) live only in `effect_decls.rs` — invisible. WANT: `:browse <Effect>` (names+sigs); a one-shot `sig`/`:i`-equivalent; an effect-verb index in the tool description (CLAUDE.md records table covers 6 types, nothing covers Time/KV/Ask/Llm).
4. **`:vocab` teaches wrong dialect + lists out-of-scope names:** displays qualified names that aren't importable (`Schemes.classifyBy`, `RustAudit.rankDesc`) and lists dead entries (`joinNL`/`splitNL`); scope = `Library` exports, vocab = directory contents, they DRIFT. Mark hidden/dead names or reconcile.
5. **Field-name lottery** (ledger #31, EXTEND to LspNode): `Hit.path/.line/.text` vs `LspNode.nodeName/.nodeFile/.nodePos` vs `Match.matchFile/.matchLine/.matchText`; CLAUDE.md trains `path/line`. → normalize to bare fields (record-dot disambiguates dups — its whole point). The bridge-generator + interface work enables this safely.
6. **Positional ADT/record rendering:** records render `{"constructor":"LspNode","fields":[…]}` — field names LOST; nested outcomes (`FileApplied [path,1,[1],[0]]`) unreadable. → render Cons as objects with field names (needs field info from the DataConTable; ties to the bridge-generator record work).
7. **Silent `[]` from search verbs:** `grepGlob`/`readGlob`/`sgFind` `[]` conflates no-hits / no-glob-match / wrong-root (missing-root IS loud — good). `""` glob = EVERYTHING (footgun, detonated a readGlob into binary files). WANT `GrepResult{hits, filesMatched, filesSearched}` / a note field.

**High (2 users):**
8. `ToJSON` teaches WRONG field names (`matchFile`→JSON key `"file"`; the toJSON-dump introspection trick then propagates the lie). 9. HasField error AVALANCHE + wrong appended hint ("add a type annotation" — real fix is field names); order the HasField error first / suppress ToJSON follow-ons. 10. DUPLICATED GHC diagnostics (`<item>:1:`+`<item>:6:`, internal `__b`/`__user` binder visible) — the remap path. 11. `ok:true` alongside `error` on failed `:commands` (breaks `ok`-scripting). 12. `:i` fails on session-defined FUNCTIONS (hint claims it searches session decls — lies) + should list ALL candidates on ambiguity (the two-`Edit`s incident). 13. `sgFind` three-way divergence (`Lang` ADT no `IsString`, `[Text]` globs vs grepGlob's single, `match*` fields, arg order).

**Notable singles:** 14. Timeout MISDIAGNOSES in-flight effects as "pure infinite loop" (`run "sleep 4"`) + unclear child-process fate → "timed out while an effect was in flight; raise timeout_secs". 15. KV = global JUNK DRAWER (~150 stale keys, no namespace/clear/TTL, cross-agent collision). 16. `readGlob` HARD-ABORTS on binary/non-UTF-8 files → want `tryReadGlob :: Text -> M [(Text, Either Text Text)]`. 17. Redefinition MERGE-vs-REPLACE asymmetry (bare eqns merge into multi-clause; sig-carrying replace), undocumented, contradicts "clauses must share one item" doc (observably false). 18. String-lets default to `forall {a}. IsString a` (want Text default). 19. wildcard `_ <- e` binds don't classify (misleading "missing do" hint). 20. `:program` replays conflicting sigs → won't compile as a module. 21. undocumented: Fs is repo-sandboxed, `**` skips dot-dirs + `target/`, `max_len` ceiling, Meta gated behind `--debug`, `:stub` usage, `editsJ` op vocabulary.

**WISHLIST (multi-user):** `:browse <Effect>`; `tryReadGlob`; `parMapM :: Int -> (a->M b) -> [a] -> M [b]` (serial mapM is the one structural regression vs bash `&`/`xargs -P`); KV namespacing; `tshow`/`inputText`; `fsRemove`; pure `decodeJson` (#27); content-hash precondition on `writeChecked` (lost-update race with parallel agents — hit LIVE this session).

**POSITIVES to PRESERVE (they begged):** the "your item is fine — a lib module is broken" message ("best error UX I've seen"); flawless error RECOVERY (nothing ever killed a session/heap — survived compile errors, `head []`, 120s timeout-abort); handler errors that encode DOMAIN expertise (sgFind sig-vs-def catch); conflicts-as-data editing (`planEdits`/`planUpdate` → `Conflicts [AnchorMissing…, RangeOutOfBounds…]`); ask/suspend/resume. **THE headline idiom all 3 named independently:** *bind the expensive search ONCE, then refine with pure one-liners forever* + *cross-effect typed JOIN through a `Map`* (`xs<-effA; let m=Map.fromList xs; ys<-effB; [enrich y m | y<-ys]`) — "that loop is the product." Document + encourage in the tool description.
...
| 35 | Minor kata-sweep frictions (2026-07-02, all low-severity / have workarounds): (a) `parseJson` polymorphic (`FromJSON a`) → a BOUND `v <- parseJson raw` is ambiguous ("Ambiguous type variable w0"); annotate or do it in one expression where a later `_Array`/`_Integer` pins the type. (b) no pure JSON decode lens `_JSON`/`_Value` (= ledger #27); use effectful `parseJson`. (c) `.|.`/`.&.` (Data.Bits) collide with `Tidepool.Effects` operators when Data.Bits is unqualified — use `Data.Bits.` qualified (all bit ops verified working). (d) KV-as-mutable-ref works (`kvSet`/`kvGet` + JSON) but is VERBOSE — a `newRef`/`readRef`/`modifyRef` stdlib wrapper (plan Phase B) would sand it. | (a) doc the annotate-parseJson idiom; (b) = #27; (c) doc or rename the Effects ops; (d) = Phase B build (do with Inanna — the JSON-typing / key-namespacing is a design choice). | 2026-07-02 |
| 34 | JIT FOOTGUN (counterintuitive): `Map.fromList` / `Set.fromList` on a large ASCENDING/sorted list (~8k+ elements) STACK-OVERFLOWS (clean yield), while the SAME on unsorted input works. GHC takes the `fromDistinctAscList` fast-path for sorted input — a non-tail balanced-tree build that hits the JIT's ~10-20k non-tail-recursion limit; unsorted input takes the `foldl' insert` path (tail-safe). So the case that is FASTER in GHC is the one that breaks here. Confirmed: `Map.fromList [(i,i)\|i<-[1..10000]]` overflows; `[((i*7919)\`mod\`100003,i)\|…]` (10000) works; `Set.fromList [1..10000]` overflows. `Map.fromListWith` is SAFE at 20k+ (insertWith fold, no fast-path) — the grep→group→sort pattern is unaffected. | Workaround: for large sorted/distinct input use `Map.fromListWith const xs` (safe path, same result for distinct keys) or `L.foldl' (\m (k,v) -> Map.insert k v m) Map.empty xs`. Consider a gotcha_registry LOUD-FAIL probe. Real fix = TCO the Data.Map build path or shadow fromList (qualified-import shadow is awkward). | 2026-07-02 |
| 32 (fixed) | A pure `let` bind whose name COLLIDES with an in-scope import (`let toList …` clashes with `Tidepool.Prelude.toList`/Map's; any Prelude re-export) failed the decl route (ambiguous occurrence) then SILENTLY fell back to materialize — producing a polymorphic `Tier1Closure` whose thin Val iface has a FREE type var → cryptic "Iface type variable out of scope: a" ONLY on a later reference (bind looked fine). Found naturally: `data Tree a` + `let toList Leaf=[]; toList (Node…)=…` + use. | FIXED (commit pending): `try_pure_bind_as_decl` now materialize-falls-back ONLY when the RHS references a materialized VALUE binding (the legit case); otherwise it surfaces the decl error (the same clean "Ambiguous occurrence" the bare-decl form gives). Guard: `colliding_pure_bind_fails_loud_not_broken`. DEEPER (open): a user still can't intentionally shadow a Prelude re-export in a decl — the real fix is hiding session-redefined names from the decl module's Prelude import (mirror the stmt plane's `hide_prelude_names`). | 2026-07-02 |
| 33 | Test/prod DIVERGENCE #2 (imports): `standalone_default` (test decl ModuleEnv) has a DIFFERENT import set than production's `session_decl_module_env` — `let toList` collides in prod (Tidepool.Prelude re-exports it) but NOT in tests (standalone uses implicit BASE Prelude, which lacks `toList`). So collision behavior can't be reproduced faithfully in tests (had to use `reverse`, a base-Prelude name, to test #32). Same class as #28 (pragmas), now for imports. | align standalone_default's imports with production, OR (better) have ONE shared decl-ModuleEnv source both use (the layer boundary blocks importing `session_decl_module_env` from runtime — may need to move the canonical env down or inject it). | 2026-07-02 |
...
| 30 (FIXED 2026-07-02) | SHOW correctness: a negative `Double` in constructor-argument position was NOT parenthesized — `show (Just (-2.5))` → `Just -2.5` (GHC: `Just (-2.5)`). Broke `read . show` round-trips; affected any structure with negative Double fields. ROOT: the `$fShowDouble_$sshowSignedFloat` interception (Translate.hs, BOTH the call-site `translate` arm ~L895 AND binding-level `emitShowDoubleSpecBody`) used `ShowDoubleAddr d` and DROPPED the `showParen (p > 6)`. FIX: new `ShowSignedDoubleAddr(prec, d)` primop parenthesizes in the RUST host (`runtime_show_signed_double_addr`, `d < 0 && prec > 6` — no hand-emitted Core comparison, so no SIGILL risk); the intercept now feeds the precedence (`args !! 1`) + Double to it. The precedence was the snag: from -O2 Core it's `I# 11#` / `appPrec1`(=11) / `minExpt`(=0, GHC CSE-shares the `0` for top-level show) — and `Resolve.hs` OVER-BLOCKED `minExpt` (its body is the bare CAF `0`, no floatToDigits), so unblocking it lets the precedence resolve. Guard: `works_show_negative_double_parens`. Validated: gotcha 35/0, sized_addr 4/0, primops-differential 25/0, real_core_corpus 1/0, haskell_suite_differential 1/0 (eval oracle agrees, no regressions). | 2026-07-02 |
